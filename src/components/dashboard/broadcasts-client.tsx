        ...
        {preview && (
          <div className="space-y-4 border-t border-gray-100 pt-4">
            ...
            <div>
              <div className="text-xs text-gray-500 mb-2">
                ...
                <span className="font-medium text-gray-900">
                  {preview.recipients.length} customers
                </span>
              </div>
            ...
            <div className="text-xs text-amber-600 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
              Note: WhatsApp may reject messages to customers who haven't
              messaged in the last 24 hours.
            </div>
            ...
            <div className="flex gap-2">
              ...
              >
                {sendMutation.isPending
                  ? "Sending..."
                  : `Send to ${preview.recipients.length} customers`}
              </Button>
        ...
        {sendMutation.isSuccess && sendMutation.data && (
          <div className="text-sm text-green-700 bg-green-50 border border-green-100 rounded-lg px-3 py-2">
            ...
            {sendMutation.data.failed > 0 &&
              ` ${sendMutation.data.failed} failed — check history for details.`}
          </div>
        )}
      </div>
      ...
      <div>
        ...
        {historyLoading ? (
          ...
          <div className="space-y-2">
            {history.map((b) => {
              const sent = b.broadcast_recipients.filter(
                (r) => r.status === "sent",
              ).length;
              const failed = b.broadcast_recipients.filter(
                (r) => r.status === "failed",
              ).length;
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${b.status === "sent" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}
                      >
                        {b.status}
                      </span>
                            ...
                            >
                              <div>
                                <span className="text-sm text-gray-900">
                                  {r.customers?.name ?? r.phone_number}
                                </span>
                                {r.customers?.area && (
                                  <span className="text-xs text-gray-400 ml-2">
                                    {r.customers.area}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`text-xs px-1.5 py-0.5 rounded-full ${r.status === "sent" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-500"}`}
                                >
                                  {r.status}
                                </span>
                              </div>
