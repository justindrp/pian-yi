...
import { activeDeliveryAreas, knownDeliveryAreas } from "@/lib/subcontractors/areas";
...
export async function GET(req: NextRequest) {
  ...
  const scope = req.nextUrl.searchParams.get("scope") === "known" ? "known" : "active";

  try {
    const db = createAdminClient();
    const data =
      scope === "known" ? await knownDeliveryAreas(db) : await activeDeliveryAreas(db);
