...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  if (!/^\d{3,5}$/.test(code)) {
    return NextResponse.json(
      { ok: false, error: "Kode akun harus 3–5 digit angka" },
      { status: 400 },
    );
  }
  if (!name) {
  ...
  if (!TYPES.includes(type as AccountType)) {
    return NextResponse.json(
      { ok: false, error: "Tipe akun tidak valid" },
      { status: 400 },
    );
  }
  if (!category) {
  ...
  if (existing) {
    return NextResponse.json(
      { ok: false, error: `Kode ${code} sudah dipakai` },
      { status: 400 },
    );
  }
  ...
  if (error || !data) {
    return NextResponse.json(
      { ok: false, error: error?.message ?? "Gagal menyimpan akun" },
      { status: 500 },
    );
  }
