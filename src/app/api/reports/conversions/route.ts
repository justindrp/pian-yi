...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const thisMonth = rows.filter(
    (r) => r.converted_at != null && r.converted_at >= monthStart,
  );
  ...
  const byCreative = Array.from(creativeMap.entries())
    .map(([creative, s]) => ({
      ...
      revenue: s.revenue,
      avgOrderValue:
        s.conversions > 0 ? Math.round(s.revenue / s.conversions) : 0,
      subscribed: s.subscribed,
      subscriptionRate:
        s.conversions > 0
          ? Math.round((s.subscribed / s.conversions) * 1000) / 10
          : 0,
    }))
    .sort((a, b) => b.conversions - a.conversions);

  const topCreative =
    byCreative.find((c) => c.creative !== "Organic")?.creative ?? null;
