...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const [
    ...
    db
      .from("orders")
      .select(
        "total_price, portions_per_delivery, package_size, created_at, status",
      )
      .gte("created_at", since),
    db
      .from("orders")
      .select("total_price, package_size")
    ...
    db
      .from("subcontractors")
      .select("id, name, late_delivery_count, total_delivery_count"),
    db
      .from("conversations")
      .select("role, input_tokens, output_tokens, model_used, created_at")
      .gte("created_at", since),
    db
      .from("customer_flags")
  ...
  const subStats = subcontractors.map((s) => {
    ...
    return {
      ...
      on_time_rate:
        total > 0 ? Math.round(((total - late) / total) * 100) : null,
    };
  });
  ...
  const sonnetOutputTokens = sonnetMessages.reduce(
    (s, c) => s + (c.output_tokens ?? 0),
    0,
  );
  const haikuInputTokens = haikuMessages.reduce(
    (s, c) => s + (c.input_tokens ?? 0),
    0,
  );
  const haikuOutputTokens = haikuMessages.reduce(
    (s, c) => s + (c.output_tokens ?? 0),
    0,
  );
  const aiCost =
  ...
  for (const d of deliveries) {
    ...
    portionsByDay[d.delivery_date] =
      (portionsByDay[d.delivery_date] ?? 0) + (d.portions ?? 0);
  }

  const totalConversations = new Set(
    conversations.map((c) => c.created_at?.slice(0, 10)),
  ).size;
    ...
    data: {
      ...
      escalationRate:
        totalConversations > 0
          ? Math.round((escalationCount / totalConversations) * 100)
          : 0,
      revenueByArea,
    },
  });
