...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  for (const delivery of toFollow) {
    ...
    const customer = delivery.customers as { phone_number: string; name: string | null } | null;
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const sentiment = (res.content[0].type === "text" ? res.content[0].text.trim().toLowerCase() : "neutral") as "positive" | "neutral" | "negative";

  await db.from("daily_deliveries").update({
    feedback_sentiment: sentiment,
    feedback_message: body.message,
  }).eq("id", body.delivery_id);

  if (sentiment === "positive") {
    await sendTextMessage(body.phone, "Senang kak suka! 😊 Kalau berkenan, boleh share ke teman-teman ya 🙏");
  } else if (sentiment === "neutral") {
    await sendTextMessage(body.phone, "Terima kasih feedbacknya kak 😊");
  } else {
    await db.from("customer_flags").update({ escalated_to_human: true, escalation_reason: "Negative feedback" }).eq("customer_id", body.customer_id);
