...
export async function POST(req: Request): Promise<Response> {
  ...
  if (grants.length === 0) {
    return NextResponse.json(
      { ok: false, error: "No grants provided" },
      { status: 400 },
    );
  }
  for (const g of grants) {
    if (
      ...
      return NextResponse.json(
        {
          ok: false,
          error: "Each grant needs customer_id, portions > 0, date, and reason",
        },
        { status: 400 },
      );
    }
  ...
  for (const id of customerIds) {
    if (!customerById.has(id)) {
      return NextResponse.json(
        { ok: false, error: `Customer ${id} not found` },
        { status: 400 },
      );
    }
  }
  ...
  for (const g of grants) {
    ...
    portionsByCustomer.set(
      id,
      (portionsByCustomer.get(id) ?? 0) + (g.portions as number),
    );
  }
  for (const [customerId, portions] of portionsByCustomer) {
    ...
    await db
      .from("customers")
      .update({
        portions_remaining: (customer?.portions_remaining ?? 0) + portions,
      })
      .eq("id", customerId);
  }
