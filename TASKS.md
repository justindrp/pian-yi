...
### `record_daily_order` drops a booking when the meal label picks the wrong order

`src/app/api/webhook/whatsapp/route.ts:2209-2271` chooses the order to draw from by matching `meal_time_preference` against the requested meal, newest first — then bails on `if (order.portions_remaining <= 0) return`. It never falls through to another order and it never uses `pickDrawOrder()`, which exists for exactly this. Nothing is written, no push is sent, and the model has already told the customer the booking is recorded.

**Vania** `+6281292339008` on 24 Agustus at 12:03–12:05 WIB booked dinner for Rabu 26, Kamis 27 and Jumat 28; the bot confirmed three times ("saya proses dulu ✅"). Her three active orders are `a2daed64` (pref null), `11b8b86d` (`lunch_only`, **10 portions left**) and `da519fd8` (`per_day_decision`, **0 left**). Only `da519fd8` is in the `dinner` preference list, so the handler took the empty order and returned. The identical exchange on 15 Agustus did write rows, because `da519fd8` still had balance then — which is why this looks intermittent. The three rows were booked by hand against `11b8b86d` on 24 Agu 17:2x WIB.

Fix: pick with `pickDrawOrder()` over every active order, preferring a meal-preference match but never letting the label exclude the only order with balance. A genuinely exhausted customer must produce a message and a push, not a silent `return`.
