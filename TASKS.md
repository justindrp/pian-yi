...
Order `35093d5c-d2be-42e1-b59d-a0514781eaa9` (Fahmi, `+6281341801449`, `dinner_only`, Dapur 1). The pause was never written (`pause_until` still null on all three of his orders) — but neither was anything else: his last delivery row was **2026-08-11**, so he was not being cooked for, he was being forgotten. On 22 Agustus he asked to resume, the bot answered *"saya jadwalkan pengiriman mulai Senin 24 Agustus ya"* and called no `record_daily_order`; nothing would have caught it — there is no nightly generator (the dead `generate-deliveries` route was deleted 2026-08-25). On 24 Agustus 18:22 WIB he asked where his food was.
...
- **WABA payment restriction `131042`.** Every business-initiated send is a dead letter: delivery proofs outside the window, `jendela_24_jam`, the `refresh-wa-window` fallback. A debit card was attached 2026-08-18 and the error only changed wording ("no payment method is set up" → "payment has been restricted"), so there is a restriction on the account itself, most likely an unpaid balance from a declined card. Clear it by hand: `business.facebook.com/billing_hub` → business `1304799927697056`, asset `1603294840784079`. Re-probe with `scripts/probe-template-window.ts`, which reads the receipt back off `conversations.whatsapp_error`. Still failing as of **2026-08-25**, now with **five** cards attached — so the cause is not a missing payment instrument but a restriction on the WABA asset, and a sixth card will not clear it. Open the link Meta's own error gives: `https://business.facebook.com/billing_hub/accounts/details/?business_id=1304799927697056&asset_id=1603294840784079&account_type=whatsapp-business-account`. Both a UTILITY (`delivery_proof`) and a MARKETING (`jendela_24_jam`) template were probed that day and failed identically, so template category is not involved; all four templates are `APPROVED` and the number is `CONNECTED`/`GREEN`/`TIER_250`, so nothing on the messaging side is a blocker either. **The send endpoint always 200s** — it returns `accepted` with a wamid and the failure arrives async in the status webhook, so never read a successful POST as proof the channel works. Verify only via `scripts/probe-template-window.ts` or `railway logs`; status payloads never reach `webhook_events` (see "No payment method on the WABA" in `WHATSAPP.md`).
...
2. ~~**The kitchen page filters it out even when it *is* captured.**~~ **Fixed 2026-08-25.** `learnCustomerContext` did record it correctly — Surya's `[AI learned context]` says "tanpa nasi (protein +25%)" twice. But `/dapur/[id]/page.tsx` strips the whole AI block and re-admits only bullets matching `PREF_BULLET`, which requires the literal label **`Preferensi`**. The summarizer was never told to emit that label; Surya's bullets are `Pemesanan aktif`, `Alamat pengiriman`, `Pelanggan atas nama Surya`, `Pengiriman siang`, `Batas order`, `Kebiasaan`. None pass. The clearest one also contains "Rp 420.000", so the `MONEY` guard would have dropped it regardless.

The filter itself is right — a dropped preference is recoverable, a leaked price is not. The bug was that the writer and the reader were built against different contracts. The summarizer prompt now requires dietary and drop-off facts to go in a bullet beginning with the exact label `Preferensi:`, with no prices in it, and the label is recorded in `learn-context.ts` as a contract with `PREF_BULLET` — change one without the other and it breaks silently again. Only *newly summarised* customers get the label, so anyone whose context was written before 25 Agustus stays invisible until their next `learnCustomerContext` run.

Break 1 is still open: with no structured field, a tanpa-nasi order still depends entirely on the summarizer happening to notice it in the chat. Surya's own note was written to `customers.notes` by hand for 26 Agustus.
