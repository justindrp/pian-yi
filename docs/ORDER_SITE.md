...
**Routes.** All in the `src/app/(catalog)/` route group, outside `(dashboard)`. `src/proxy.ts` gates nothing — it only refreshes the session — so **these pages are public the moment their own layout stops checking**. Until launch, `(catalog)/layout.tsx` 404s anyone not signed in as an admin and sets `noindex`; launching is deleting that check and the `robots` line, nothing else. All rendered from rows filtered on `is_active = true` through `loadCatalog()` (`src/lib/catalog/kitchens.ts`), so deactivating a kitchen removes it everywhere at once, and a kitchen with no `customer_nickname` or no ladder is left off rather than shown half-built. The `CatalogKitchen` it returns carries no id and no real name, so a page cannot print one by accident.

| Route | What |
| --- | --- |
| `/menu` | Every active kitchen, cheapest first, with the areas as links. The comparison surface. Built: area links. Not yet: filters by day, price band, style. |
...
4. **The public catalog** — `/menu`, `/menu/[dapur]`, `/harga`, `/area/[area]`, each ending in a click-to-chat. No token, no personal data, no writes. **Built ahead of phases 1–3 as an admin-only preview** (2026-09-24), because everything but the menu already had rows; it launches when the two blockers under "Open before this starts" clear, and gains menus when phase 3 lands. It follows the food-delivery-app layout approved on 2026-09-23 (home list with area picker and filter chips, kitchen page with Paket / Menu / Info tabs, bottom tab bar), minus every part of that design no row backs yet: kitchen photos (a flat colour stands in), a line of identity copy per kitchen, the weekly menu and the "Menu minggu ini" strip, search, and the Pesananku tab (phase 5). A package's **Pesan** opens WhatsApp with that package typed out; the cart, checkout and payment screens are phases 6–7. Verify: nothing on any page names a kitchen or reproduces its own copy; every figure traces to a row; the pages render with one kitchen active and with twenty.
