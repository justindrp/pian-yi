...
- **The rebrand.** "Pian Yi" (便宜, cheap) anchors a price band we have already left. `business_name` is a setting the prompt reads, but there are ~41 hardcoded strings in `src` and `scripts`, plus the invoice renderer, the manifest and the legal pages — and the WhatsApp Display Name change goes through Meta review, which is the slow part. **The name is Katerloka** (kater + *loka*, "the world of catering"), chosen 2026-09-23 over descriptive names like KateringHub, which DJKI would likely refuse under Article 20 of UU 20/2016. PDKI's normal search found no match that day; the phonetic search and the filing itself (classes 35 and 43, first-to-file) are still to do. No domain bought yet — katerloka.com and katerloka.id were both free on 2026-09-23 — and until one is, the site runs on the Railway domain.
