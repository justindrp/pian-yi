...
4. **The public catalog** — `/menu`, `/menu/[dapur]`, `/harga`, `/area/[area]`, each ending in a click-to-chat. No token, no personal data, no writes. **Built ahead of phases 1–3 as an admin-only preview** (2026-09-24), because everything but the menu already had rows; it launches when the two blockers under "Open before this starts" clear, and gains menus when phase 3 lands. Verify: nothing on any page names a kitchen or reproduces its own copy; every figure traces to a row; the pages render with one kitchen active and with twenty.
