...
The plan for moving order intake off the chatbot and onto a web configurator the bot links to from inside the WhatsApp thread. Written 2026-09-09. Nothing here is built yet.
...
It is a **configurator the bot links to**, not a website customers visit. The bot sends a tokenised link inside the conversation, the customer taps it, fills in what a form can express better than a sentence, and comes back to the thread. WhatsApp stays the transport and stays the relationship.

It is **not a destination**. No homepage funnel, no "visit our site to order", no login. Sending this audience to a site to start a relationship is where conversion dies; the landing page at `/` exists for Meta's business verification and for search, and that is all it is for.

It is **not a marketplace**. The kitchen anonymity decision of 2026-09-06 stands: customers see `subcontractors.customer_nickname`, never a real name. The site therefore presents kitchens the way the bot does — Dapur Suplir, Dapur Palem, Dapur Monstera — with a menu identity to choose on, not a supplier to look up.
...
**The price sheet images stay.** They do browsing, not quoting: a forwardable, zero-tap artifact that renders in the thread, survives offline and is the thing someone screenshots for their spouse. A form cannot do that. What breaks at ten kitchens is the delivery mechanism, not the format — you cannot send ten images. So `send_price_list` becomes a comparison (one composite, or the link) plus the individual sheet on request: *"boleh lihat harga Dapur Palem?"* → that one image. `scripts/price-list.ts` also needs a second layout, because Homey's real card is a drop-size table and the generator only knows how to draw a package-size ladder.
...
## The two tables this needs behind it
...
3. **`order_links` + `/pesan/[token]`** (migration 107) rendering read-only: who you are, your kitchen, your quota. No writes. Verify: an expired and a nonexistent token are indistinguishable; nothing renders for a token that is not yours.
4. **The configurator**, steps 1–5, ending at the existing manual-transfer instructions. `send_order_link` added to the bot; `extract_order` still live behind it. Verify: an order placed through the form is byte-identical in the database to the same order placed through the bot.
5. **Checkout** behind the adapter, one provider. Verify against the sandbox: paid callback stamps `paid_at`, posts the journal and writes the delivery rows; a replayed callback changes nothing.
6. **Retire `extract_order`** and its prompt machinery once the form has carried real orders for a fortnight. Verify: the bot cannot create an order at all — the only path is the link.
7. **`send_price_list` rework** and the drop-size sheet layout.

Phases 1 and 2 are worth doing whatever happens to the rest: they are the multi-kitchen architecture debt, and the form only makes them urgent.
...
- Kitchen real names never reach a customer, on any page, in any error, in any URL.
...
- **Menu identity per kitchen.** "Dapur Monstera" and "Dapur Suplir" give a customer no basis to choose between two kitchens priced 55–65% apart. The form makes that gap visible in a way the chat never did, so the copy has to exist before step 3 is worth shipping.
