...
| price_list_image_url | text | This kitchen's own price list sheet, rendered by `scripts/price-list.ts --upload` (migration 099). NULL falls back to `settings.price_list_image_url`, the same way `subcontractor_id IS NULL` is the fallback ladder. One global image stopped being right the moment migration 098 gave each kitchen its own ladder: the house sheet's bottom tier is Rp 29.000 against Dapur Monstera's real Rp 45.000, and the welcome sequence sends that sheet before the customer has said anything |
