...
| same_menu_both_meals | boolean | Whether this kitchen cooks the same menu for siang and malam (migration 097). True for Thenie only today. Read the same way as `offers_size_m`: the bot's prompt builds its sentence from `dapurOptions[].sameMenuBothMeals` and names exactly the kitchens that have it, saying nothing when none do. This used to be a prompt line naming Dapur 1 — a fact about Thenie, not about whichever kitchen is listed first, so it would have lied the moment the kitchen was renamed and stayed silent for the next kitchen that shares its menus |
| lunch_window_start_min / lunch_window_end_min | int | When this kitchen's courier is at the door for lunch, in minutes from midnight WIB (migration 093). Null on both means unmeasured and `DELIVERY_WINDOWS` in `src/lib/deliveries/windows.ts` applies — half a pair is not a window and also falls back. Minutes, not hours, because Dapur 1 arrives **11.30-12.30** and 12.30 has no hour-resolution spelling. Every customer of theirs used to be quoted 10.00-12.00, which is how Naya was told at 11.09 on 2026-09-02 that her food was late when it was 21 minutes early |
