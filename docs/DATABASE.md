...
There are **two Superbank accounts**, and neither belongs to the business: `000076157940` is Annie's (ANGELA OCTAVIANI), which carried the kitchen payments Desember 2025 – Juni 2026, and `000091762385` is Agnes's, which carried them from Juli 2026 until she resigned. Annie has no Juli 2026 e-statement and never will have one that matters: her last transaction on that account was 3 Juli, and Superbank does not publish a month's PDF until the 1st of the next month, so the account was already handed over before the file existed. A pair of app screenshots covering 1–3 Juli is therefore the complete record of the month, not a stopgap for a missing one — import them as `source = 'screenshot'`, which is exactly the case that value is for.
