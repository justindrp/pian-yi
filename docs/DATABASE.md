...
**2003 Partner Current Account - Annie holds her profit share, and that share is a draw rather than a cost** (migration 103). She takes 40% of gross profit — revenue minus that day's subcontractor cost, the same split `GET /api/reports` prints as `annieShare` — and she took it **daily**, not at period close, so it reaches the statements as 143 small transfers to "Angela Octaviani" in 108 distinct odd amounts, Rp 12.658.400 between Desember 2025 and 3 Juli 2026. Nothing about such a line says what it is; only the counterparty does, which is why they read as personal transfers until you know. Booking them as expense would understate profit twice — once as a cost, then again as her share of what was left. They cannot go in 2002 either: that account is Justin's, and its balance is the running answer to what the business owes *him*. So 2003 swings the same two ways beside it — her draws and her personal card purchases on the business account debit it, money she sends back credits it, and her earned share credits it at period close. A debit balance means she has drawn ahead of what she has earned. `scripts/classify-annie-draws.ts` points the lines at it and skips any a human has already set by hand.
...
| counterparty | text | As the bank spells it, which is not how the customer spells it. Taken from the fragment before the mutasi amount, never the last fragment in the record — see below |
| description | text | |
...
**The transaction ends at the mutasi amount, and the counterparty is the fragment before it.** A BCA record is its keyword line plus every wrapped line after it, up to the next keyword line — so the last record on a page swallows everything printed below it: the letter-spaced branch name `K C U B U M I S E R P O N G D A M A I`, our own name and address, and the CATATAN block ending in `INDONESIA`. The parser used to take the last name-shaped fragment in the record, which on those records is page furniture, so **90 lines of 1925 were filed under a counterparty of `INDONESIA`** — including six 2025 customer debits the backfill then could not match to an eater. Eight more read `/DBS MOBILE`, `/MB BPD PAPU`, `/Mobile Bank`, `/PLAZA MANDI`: on a `SWITCHING` line the sending bank's channel is printed *after* the sender, so the last fragment is the channel. Both are the same mistake — the end of the record is not the end of the transaction. Cut the record at the part that carries the amount, take the last name fragment before it, and ignore any fragment opening with `/`. Fixed 2026-09-14 and every statement re-imported: 123 counterparties corrected, no amount, date or line count moved, all control totals still tie. A bank fee now carries a null counterparty instead of a copy of its own line (`BIAYA ADM 10,000.00 DB 2,619,022.52`), which every consumer already handles — they read `counterparty ?? description`.

**A Superbank PDF is several accounts, not one.** "Tabungan Utama" comes first, then whichever pockets exist — named ones the holder created ("Saku Catering PianYi", "Saku Tabungan Ani") and the app's own savings products (OVO Nabung, Celengan, Deposito, Kartu Untung). Every pocket prints rows in the same shape as the main account, so reading the file as a single account folds them together and the control totals miss by exactly the pockets' turnover: Rp 4.088 in Desember 2025, Rp 7.932.233 in Mei 2026, which is what refused seven of Annie's statements on the first import. `parseSuperbankStatement()` therefore returns one `ParsedStatement` per section, each with its own account number and its own closing totals line, and they are stored as separate `bank_statements` rows. The pockets are not noise to be discarded — "Saku Catering PianYi" is where the kitchens were paid from Mei 2026 onwards.

There are **two Superbank accounts**, and neither belongs to the business: `000076157940` is Annie's (ANGELA OCTAVIANI), which carried the kitchen payments Desember 2025 – Juni 2026, and `000091762385` is Agnes's, which carried them from Juli 2026 until she resigned. Annie has no Juli 2026 e-statement and never will have one that matters: her last transaction on that account was 3 Juli, and Superbank does not publish a month's PDF until the 1st of the next month, so the account was already handed over before the file existed. A pair of app screenshots covering 1–3 Juli is therefore the complete record of the month, not a stopgap for a missing one, and they are stored as `source = 'screenshot'` — exactly the case that value is for — by `scripts/import-annie-july-2026.ts`. A screenshot carries no control totals, so the script checks the only thing it can: the eight lines of the main account run its Juni closing balance of 312.354,43 down to 0,43, and the pocket's three run 5.637,23 up to the 34.991,23 the app was showing. Both tie to the sen, which is what proves no row was hidden above or below the fold — the check a screenshot import must always have, because scrolling is the way it loses data.

**A null `journal_id` on a 2100 credit does not mean the payment is unbooked.** `order_payment` journals are posted from the `orders` table and never wrote the link back, so 31 of the 64 unjournalised credits already had a journal when the statements were first imported. Posting a journal for every unlinked row would have double-counted Rp 14M+ of revenue. Linking is therefore its own pass and runs first: `scripts/link-bank-journals.ts` (dry run by default, `--apply` writes) stamps `journal_id` on rows whose journal already exists and creates nothing. It links only where the amount, the date within three days, **and the name** all agree — uniqueness inside the candidate set is not proof, because a deposit whose own journal is still missing can be the sole amount-and-date match for someone else's, and a draft of the script duly offered to file Dessy's Rp 174.000 against Veronica's JV. A sole candidate whose name disagrees is printed for review, not linked; several of those are genuine proxy payers.

**Posting the genuinely-missing ones is phase 2, `scripts/post-bank-receipts.ts`** (dry run by default, `--apply` writes), keyed `bank_receipt` on the `bank_transactions` row so a re-run writes nothing. What it refuses to touch is the whole design:

- **A line with any plausible journal is held, not posted.** The test is not "did phase 1 link it" — phase 1 links only what it can prove, and the deposits it could not prove are exactly the ones a naive phase 2 would double-post. Same bank account, same amount, within three days, **whatever the journal's `source_type`**: the search deliberately includes journals already claimed by another line and hand-typed `manual` ones. Carolin's Rp 29.000 ongkir was posted by hand as JV-2026-640 with no order behind it, and a guard that only looked at `order_payment` would have booked that deposit twice. 41 lines were held on the first run; a human resolves them, no rule can.
- **Bank-to-bank contra accounts are excluded** (1002, 1003, 1005, 1006). An internal transfer appears twice in the statements — the debit leaving one account and the credit arriving in the other — and both lines are classified, so posting both books one movement twice. Which side is the correct one depends on which statements exist: ShopeePay has 108 debit lines against it and no statement of its own, so posting only the arriving side would lose Rp 151jt of float movement. That decision belongs with the money-out pass.
- **No `BOOKS_START` cutoff, unlike `settleBankLines()`.** The asymmetry is deliberate and must not be "tidied": a kitchen payment before the books began clears a payable that was never accrued and leaves 2001 a debit reading as the kitchen owing us money, whereas a customer deposit *creates* a liability the delivery journals later clear. Posting the full Sep-2025 history is correct on its own terms, and 2100 carries that period as deferred revenue until the Dec-2025 – Jun-2026 accrual replay lands.
...
**The link runs both ways.** Pull: the recipient asks and gets the photo. Push: every proof we send that customer is sent to this number too — `sendDeliveryPhotoToCustomer()` reads this table on every send (never cached, so a deleted row stops the photos at the next delivery) and addresses one uploaded media id at each number. Each recipient's 24-hour window is measured on **their own** last inbound, so one number may get the free-form photo with its caption while the other gets the `delivery_proof` template. A recipient with no `customers` row has no conversation to write the send into; it is recorded in `edit_log` against this row (`entity_type: "customer_contacts"`, `action: "send"`) instead, and no customer row is created to fix that.
...
| created_at | timestamp | |
| updated_at | timestamptz | Migration 129, maintained by the `set_updated_at()` trigger. The inbox's flag sweep polls it instead of refetching — see "The inbox refresh" in `ADMIN.md`. Before it existed the table had no timestamp, so a takeover moved no watermark and the sweep had to refetch blind |
...
| collects_from_courier | boolean | The customer meets the courier and takes the box by hand, because their building refuses a lobby drop (Synergy: Naya, Cila, Winy, set 2026-09-02). A courier photographs what he leaves, not what he hands over, so these customers routinely have no `delivery_proofs` row for food they ate — 2 of Naya's first 3. `send_delivery_proof` therefore never tells them the food was not delivered; it says we hold no photo and asks whether the courier has been in touch |
...
| customer_id | uuid | FK → customers |
| subcontractor_id | uuid | FK → subcontractors — which kitchen fulfills this delivery. It is the row's own kitchen, not the order's: a package may be split across kitchens, so this can differ from `orders.subcontractor_id` |
| price_per_portion | integer | Nullable (migration 102). What this delivery is worth per portion when its kitchen is not the order's; NULL means `orders.price_per_portion`, which is the ordinary case. Revenue recognition draws 2100 down per row, so an away day has to carry its own rate or the difference stays stranded in the liability. Frozen at order creation from `requested_schedule`, and `POST /api/deliveries/daily-sheet` reads the prior row and writes it back so an admin re-submitting a sheet does not reset it |
| delivery_date | date | Date of delivery (YYYY-MM-DD) |
| meal_type | text | "lunch", "dinner" or "breakfast". `CHECK (meal_type <> 'both')` since migration 114; `UNIQUE (delivery_date, customer_id, meal_type)` limits a customer to one row per slot per day. **One row is one meal.** "both" used to be writable and was invisible food: every consumer buckets by an equality test, so a row carrying two meals matched no column, reached no kitchen and was counted in no total, while still spending the quota — the ledger read "booked" for food nobody cooked. `record_daily_order` passed the bot's meal_type straight through, and three rows reached production (Vania 17 and 24 Juli, Veronica Catherine 14 September); all three are deleted, the portions returned, and the tool enum, the `extract_order` recovery extractor and the simulator now offer one meal per call. "breakfast" is real and stays — event bookings with a morning run (ICE BSD, 21–23 Agustus 2026); no standing package produces one. Anything reading this column must handle every value: `/dapur/[id]` filtered on lunch/dinner only and rendered the morning rows nowhere, and `deliveries-client.tsx` still buckets lunch/dinner only, so an event's breakfast rows do not appear on the admin sheet |
...
| size | text | Portion size: `"s"` (menu items 1, 2, 3, 5) or `"m"` (all five — one extra side dish) — default `"s"`, constraint `IN ('s', 'm')`. Read it through `normalizeSize()` (`src/lib/orders/size.ts`): legacy rows and a model-filled field both reach `null`, and anything that is not the literal `"m"` is S. M adds that kitchen's `subcontractors.size_m_surcharge` (house setting when NULL) to `price_per_portion` at creation and is only sold by a kitchen with `offers_size_m` |
...
| no_rice | boolean | The customer bought this package tanpa nasi (migration 116), default false. `subcontractors.no_rice_discount` is subtracted per portion and frozen into `price_per_portion` at creation, so this column is not what prices the order — it is what lets `resizePendingOrderFromMessage()` re-price an amended portion count with the discount still on. The cook still reads the request from `customers.kitchen_notes`, which `extract_order` back-fills with "tanpa nasi" when the model sends the flag and no `catatan`. **Orders that pre-date the column were rescued from those same notes** (migration 117): `default false` was right for a new order and wrong for every existing one, so the morning after 116 shipped the flag read false on all 324 active orders including 13 tanpa-nasi ones. The backfill only touched orders still live — `active`, `paused`, `payment_proof_received`, `pending_payment` — because `kitchen_notes` is customer-level and current, and a finished order's notes may describe how the customer eats now rather than what they bought in June |
| amount_paid | integer | IDR received against this order so far, default 0. For DP / partial payment on corporate orders; `total_price` stays the contracted amount. Set by an admin with **Catat DP** on `/payments` or in the Orders slide-over ("Sudah dibayar") — nothing derives it and nothing gates on it. `/payments` shows it as "DP … · sisa …" while `paid_at` is null |
...
| requested_schedule | jsonb | Nullable. The days the customer asked for: `[{date, meal_type, portions}]`, plus `subcontractor_id` and `price_per_portion` on the days another kitchen cooks (2026-09-06) — home days stay bare, and the away rate is frozen there so a later ladder edit cannot reprice an order the customer already agreed to. Written once at order creation, read once by `mark_paid` to write `daily_deliveries`, never read again — the rows are the truth from there. Null means they bought quota without naming days and book per date — that is what `extract_order`'s `delivery_schedule: []` stores, and it is a normal order, not a gap. `mark_paid` filters the days through `isDeliveryDay()` and `isLocked()`, so a Minggu, a libur nasional, or a date whose H-1 cutoff passed before payment is dropped and its portions stay unbooked. Replaced `meal_time_preference`, dropped in migration 077: a single enum could not express a per-day schedule, and reading a delivery row against it produced a false bug report on correct food. |
| custom_schedule | json | Per-weekday schedule if preference is "custom_schedule" |
...
| payment_proof_read | jsonb | What the model read off the slip image (migration 105), so an admin verifying at `/payments` sees the figure without opening the picture. `{amount_idr, recipient_name, bank, datetime, is_transfer_receipt, matches_total, read_at}`. **Advisory only — nothing pays on it.** `paid_at` stays a human decision, because marking paid writes `daily_deliveries` rows and nothing filters the kitchen sheet by order status, so a misread or forged screenshot would become cooked food. `matches_total` counts the **sum** of the orders the proof was applied to as a match, not only a single total: one transfer routinely covers two orders, and a slip read as 685.000 against orders of 540.000 and 145.000 is a correct read. It is `null`, never `false`, when the figure did not read. `amount_idr` is dropped above Rp 100 juta — a thousand-separator misread put 1 slip in 36 at 145.000.000 for a 145.000 transfer, and a figure that large shown as if it had been read is worse than none. Written by `handlePaymentProofImage` after the proof is already banked, so a model outage costs the pre-read and nothing else. |
...
| Column | Type | Notes |
|--------|------|-------|
| subcontractor_id | uuid | The kitchen this ladder belongs to, FK to `subcontractors` (migration 098). **NOT NULL since migration 136** — there is no house ladder. The twelve NULL rows were always exactly Thenie's (Dapur Suplir's) prices: 135 copied them onto her id unchanged, and 136 deleted the originals in a later push, so the build still serving during the deploy never lost her ladder. Trigger `active_kitchen_needs_ladder` on `subcontractors` refuses to activate a kitchen with no rows here. Unique on `(subcontractor_id, portions)` with `NULLS NOT DISTINCT`, which replaced the `portions` primary key |
...
**Every read goes through `tiersForKitchen()` (`src/lib/pricing/tiers.ts`), never a bare select.** One ladder for the whole business was true while one kitchen cooked everything and stopped being true the moment a second one did: Thenie costs us Rp 21.000 a portion, Santapin Rp 22.000, Homey Rp 33.000, so selling Homey's food at Thenie's Rp 28.000 tier is a Rp 5.000 loss on every portion and nothing in the order looks wrong. A bare select now returns every kitchen's rows interleaved and the largest-tier-below lookup on top of it quotes whichever kitchen sorts first. Writes are scoped the same way: `PATCH /api/settings/pricing` keyed on `portions` alone, which would have repriced every kitchen's row at that size in one request, and now requires a `subcontractor_id` and writes only that kitchen's rows; `/settings` picks the kitchen with a row of pills.

Dapur Suplir (Thenie): 5→29k, 6→29k, 10→28k, 12→28k, 20→27k, 24→27k, 40→26k, 48→26k, 60→26k, 72→26k, 120→25k, 144→25k. Every other kitchen carries its own twelve rows, anchored at 25% margin on the 10/12 tier (see "Per-kitchen price ladders" in `OPERATIONS.md`).
...
Notable keys: `business_name`, `chatbot_enabled`, `welcome_message`, `price_list_image_url`, `bank_name`, `bank_account_number`, `bank_account_name`, `order_deadline_hour`, `size_m_surcharge` (rupiah added per portion for size M, on top of the S tier or a contract rate — Rp 4.000; a missing or unparseable value reads 0 and prices M as S. **A fallback since migration 131**, not the figure: a kitchen states its own in `subcontractors.size_m_surcharge` and this applies only where that column is NULL or no kitchen is known), `task_wip_limit` (how many tasks may be `in_progress` at once — 3, Justin's own capacity rather than the customary kanban 5, which is a headcount number; a missing or unparseable value falls back to 3 rather than to unlimited), `task_reminder_phones` (comma-separated handsets that get the 07:00 WIB task reminder, migration 128; ships **empty**, so the cron falls back to push until somebody fills it in at `/settings` — and it should hold the same numbers as `proof_forwarder_phones`, because a handset that is not a proof forwarder is read by the webhook as a customer and replying to the reminder would open a customer thread), `casual_mode_probability`, `typing_delay_base_seconds`, `escalation_keywords`, `instagram_handle`, `team_roster` (who works for us, one per line "Nama — peran" with the number they contact people from; the bot confirms these people to a customer asking whether someone is ours, and the validator does not read their names as a claim about the customer — migration 137, ships **empty** so no personal number is in the repository; see "The bot confirms our own team, from a setting" in `docs/BOT_RULES.md`), `admin_display_name` (the one admin the bot may name to a customer when it hands off — "Kak Justin"; empty means it says "tim admin kami" and names nobody, migration 087), `whatsapp_business_number` (the WABA the app sends through), `whatsapp_manual_number` (the hand-operated second account, migration 081) and `proof_forwarder_phones` (comma-separated numbers allowed to forward a delivery photo to the WABA and have it sent on to the customer named in the caption, migration 083; the manual number was added by 088, which appends rather than overwrites because the list is edited in the UI. It is also the only "this number is never a customer" list there is — the webhook checks it **before** the customer upsert, so a listed handset gets no bot reply and no `customers` row, and a staff phone that forwards nothing is still put on it for that reason. One listed after it had already chatted the bot keeps its old row, so `review-leads` and `review-chats` filter on the list too — see "Proof Relay" in `docs/ADMIN.md`). Of the two numbers, `whatsapp_manual_number` is read by `windowWarning()` in `src/lib/deliveries/forwarded-proof.ts`, which names it to an admin whose forwarded proof went to a closed window, and `whatsapp_business_number` is read by `manualDraft()` in the same file, which puts it in the paste-ready message asking a closed-window customer to chat the main number. The manual one must never be added to `src/lib/claude/prompts/system.ts`, which reads settings by name and would hand customers a channel with none of the API path's guards. See "The manual number" in `docs/WHATSAPP.md`

**`settings_cache_watermark()` is how the server decides whether to reload any of this** (migration 129). `src/lib/cache/settings.ts` holds `settings`, `message_templates`, the active `chatbot_instructions` and `area_neighborhoods` in memory — and held the house `pricing_tiers` rows until migration 135 removed them and dropped the table from the watermark — and used to re-select all of them every 60 seconds — 20,621 bytes a refresh, 1,440 times a day, 849 MB a month, whether or not anyone was using the app and whether or not anything had changed. The function returns `count:max(updated_at)` per table as one 166-byte string; the tick reloads only when it differs from the mark the cache was built against.

Two things make that safe, and both are load-bearing:

- **`updated_at` is maintained by a trigger, not by the write paths.** This project has no other triggers and `updated_at` is a plain `default now()`, which fires on insert and never again. Every writer here — the settings upsert, `PATCH /api/settings/templates`, `PATCH /api/settings/pricing` — leaves the column at its original value, so a `max(updated_at)` watermark over them would never move and the bot would serve a frozen price list forever. `set_updated_at()` is attached `before update` to the four cached tables, plus `pricing_tiers` (no longer cached) and `customer_flags`. **Do not remove it and do not add a fifth cached table without attaching it**, or that table's edits stop reaching the bot silently.
- **The count is there for deletes.** A trigger plus `default now()` covers inserts and updates, but a `DELETE` moves `max(updated_at)` only when the deleted row happened to be the newest. Deleting a neighbourhood or a chatbot instruction is ordinary here, so the mark carries the row count too.

A watermark that cannot be read is **not** treated as unchanged: `readWatermark()` returns null on any error and the cache falls back to reloading every minute, which is what it did before. That is what makes a deploy landing ahead of this migration a cost problem rather than an outage.
...
| customer_nickname | text | Customer-facing name shown in the chatbot. Ornamental-plant names since 2026-09-04 — Thenie is "Dapur Suplir", Santapin "Dapur Palem" — replacing the numbered "Dapur 1/2/3/C/D". The numbers implied a ranking nobody meant and had to be renumbered whenever a kitchen was added or dropped; a plant name says nothing about the food, the cuisine or the quality, so it never has to change and never leaks anything about the kitchen behind it. Read it from the row: never write a nickname into a prompt, a doc or an `if` |
| admin_phone | text | Primary WhatsApp number for the kitchen admin |
| admin_phone_2 | text | Secondary WhatsApp number |
| delivery_areas | json | Array of area strings **this kitchen** serves. Each kitchen has its own list and the lists overlap only in part; the areas Pian Yi offers are the union of this column across rows with `is_active = true`, computed at read time by `activeDeliveryAreas()` / `unionAreas()` (`src/lib/subcontractors/areas.ts`) and never stored anywhere else. **This column is the only source** — not `settings.delivery_areas` (which exists, is written by nothing that reads it, and is no longer editable in the UI), and not a literal in code. Editing this column, or flipping `is_active`, changes what the chatbot tells customers. See "Delivery areas" in `OPERATIONS.md` . **It is jsonb, so it will accept anything that is valid JSON** — write a list as `'["Bintaro"]'::jsonb` and an empty one as `'[]'::jsonb`. Migration 113 wrote `'{}'`, the Postgres array literal that is correct for `delivery_days` and wrong here, and jsonb stored an empty *object*: `?? []` does not catch an object, so `.join` threw and the Settings Subcontractors tab went blank for every admin over an inactive kitchen used once in Desember 2025. Migration 115 fixed the row and added a `jsonb_typeof = 'array'` check constraint; every reader goes through `asAreas()` (`src/lib/subcontractors/areas.ts`), which returns `[]` for anything that is not a list |
| cost_per_portion | integer | What we pay per portion in IDR — used for Route 2 (kitchen delivers) |
| no_rice_discount | integer | IDR off per portion for a tanpa-nasi box at this kitchen, flat across every tier (migration 098). It was a cost column read by nothing until 2026-09-15, when Justin decided the saving is passed through: `getExtractedOrderPricing()` subtracts it from the ladder rate and the prompt quotes the figure per dapur, so it is now what the customer pays as well as what the kitchen knocks off us (migration 116). Never off a `contract_price_per_portion`. **NULL or 0 means this kitchen charges the same either way, never that it refuses** — every dapur accepts tanpa nasi, and refusing it is what lost the 2026-08-26 lead who asked "kl hanya lauknya bisa kak ?". Thenie's is NULL, Santapin knock off Rp 2.000 — the column said 2.500 until 2026-09-06, which matched neither their cost gap (22.000 with rice, 20.000 without) nor the marked-up version of it. **Homey cannot be expressed here and their Rp 4.000 is the least-wrong single value**: their gap moves with how many portions one drop carries — Rp 4.000 at 1 portion, Rp 5.500 at 2, Rp 4.333 at 3, Rp 4.750 at 4, Rp 4.200 at 5 — so a single integer is the wrong shape for them. It held a placeholder Rp 5.000 until 2026-09-06, when Homey's public price card confirmed the ladder; 4.000 is the 1-portion gap, and 78% of drops carry one portion |
| price_list_image_url | text | This kitchen's own price list sheet, rendered by `scripts/price-list.ts --upload` (migration 099). **Required while `is_active`** (check constraint `active_kitchen_needs_price_list_image`, migration 134); until then NULL fell back to `settings.price_list_image_url`, which is Thenie's ladder. One global image stopped being right the moment migration 098 gave each kitchen its own ladder: the house sheet's bottom tier is Rp 29.000 against Dapur Monstera's real Rp 45.000, and the welcome sequence sends that sheet before the customer has said anything |
| delivery_days | smallint[] | ISO weekdays this kitchen cooks, 1 = Senin (migration 098, default `{1,2,3,4,5,6}`). Senin–Sabtu was a fact about the business because Thenie work Saturdays; Homey do not — their menu grid has five columns and no Sabtu, so a Saturday row on their sheet is food nobody is cooking. This is the whole weekday calendar for that kitchen, and it widens as well as narrows: Santapin lists 7, so their Minggu is a working day. Pass the list to `isDeliveryDay(ymd, days)` — with no list it falls back to `BUSINESS_DAYS` (Senin–Sabtu). Only the libur nasional close for everybody |
...
| offers_size_m | boolean | Whether this kitchen cooks size M (migration 078). Per kitchen, like `delivery_areas` — Thenie today and Molls from migration 132, and nothing may hardcode that. The bot's prompt builds its size section from this column, and `createOrderFromExtraction` re-reads it after resolving the kitchen and downgrades an M order to S rather than sending an M row to a kitchen that cooks S |
...
| msg_policy | text | How this kitchen seasons (migration 124), constrained to three values. `none` = cooks without MSG. `penyedap` = no pure micin, but a flavour enhancer — kaldu bubuk or kaldu jamur — so the food is not MSG-free either. `msg` = micin used directly. **NULL = nobody has ever asked that kitchen, which is not `none`**. Homey is `none`; Thenie is `penyedap` because they season with kaldu jamur merek Totole, which is how an admin answered the Lippo Karawaci lead on 2026-09-09 and where this fact came from. Santapin is NULL and stays NULL until someone asks them. **It was a boolean for one day** — `uses_msg`, added by migration 123 and dropped by 125 once the code reading `msg_policy` was live — and a boolean put Thenie at the wrong end: `true` reads as "we cook with micin" and `false` as "the food is free of flavour enhancer", and both are a lie to the person asking. The prompt renders it from `dapurOptions[].msgPolicy` by nickname, says both halves of the middle state and is forbidden to shorten either way, and escalates with `ask_admin_for_help` on a NULL. The brand is recorded here and deliberately never reaches the prompt: a customer asking about MSG is asking what is in the food, not which supplier we buy from. The question had no answer anywhere in the prompt until 2026-09-19, and four customers asked it between 1 and 17 September |
| lunch_window_start_min / lunch_window_end_min | int | When this kitchen's courier is at the door for lunch, in minutes from midnight WIB (migration 093). Null on both means unmeasured and `DELIVERY_WINDOWS` in `src/lib/deliveries/windows.ts` applies — half a pair is not a window and also falls back. Minutes, not hours, because Thenie arrives **11.30-12.30** and 12.30 has no hour-resolution spelling. Every customer of theirs used to be quoted 10.00-12.00, which is how Naya was told at 11.09 on 2026-09-02 that her food was late when it was 21 minutes early |
| dinner_window_start_min / dinner_window_end_min | int | The same for dinner. Unset for every kitchen so far, so dinner still reads 16.00-18.00 from the fallback |
...
| size_m_surcharge | integer | What **the customer** pays per portion for M at this kitchen, on top of the S tier and on top of a contract rate (migration 131). NULL falls back to `settings.size_m_surcharge` (Rp 4.000). Thenie carries 4.000 on her own row since migration 133 rather than borrowing the setting, and Molls 6.500 — they charge us Rp 5.000 more for the M dish, so quoting them off the global setting loses Rp 2.500 a portion. A stored **0** is honoured ("M at the S price") rather than read as missing. This is the sell side; `cost_per_portion_m` below is the buy side, and the two are not derived from each other. Read it with `kitchenMSurcharge(db, subcontractorId)` (`src/lib/pricing/tiers.ts`) or `sizeMSurcharge(sub)` when the row is loaded |
...
| catalog_blurb | text | One public line describing this kitchen's food and delivery, shown under its nickname on the catalog (migration 138). **Public text: never the kitchen's name, neighbourhood or packaging** — anything a customer could search to find who cooks. Written from `menu_text` and `notes` for the first four kitchens; edited in the kitchen's dialog at `/subcontractors`. Null shows nothing |
...
| takes_events | boolean | Whether this kitchen may be tendered a **one-off event** (migration 106). **An overlay on `is_active`, never a partition of it** — daily and event are not exclusive, so a kitchen can carry both flags, one, or neither, and nothing here forces an either/or the business does not have. It exists because `is_active = false` had come to mean three unrelated things at once: prospects we priced and never signed (Mama Echa, Molls, Perut Bahagia, Yuk Makan), leads never onboarded (cost 0, no phone), and Dapur Uma Ceo — a kitchen that had actually cooked for us, distinguishable from the other ten only by a paragraph someone typed into `notes`. Who to tender the next event to is a query now, not prose. Do **not** wire this into `activeDeliveryAreas()`, the ladders or the kitchen sheets: an event is tendered and hand-priced, it never draws on `pricing_tiers` and it never writes a `daily_deliveries` row |
...
The work queue, replacing the old `TASKS.md` on 2026-08-25. A file only Claude could update went stale between sessions and no other admin could ever see it. Edited at `/tasks`, printed by `pnpm tasks`. Statuses are `open | in_progress | blocked | done`, now held by `tasks_status_check` (migration 107) as well as by the allowlist in `src/app/api/tasks/validate.ts`; adding a status means the constraint, that allowlist, `STATUS_RANK` and `BAND`. The constraint is what migration 072 declined to write — "a convention between three people, not a state machine" — and the convention drifted: 32 rows accumulated on `todo` between 1 and 9 September, a second spelling of `open` with no filter chip, so they were invisible under every filter but "All". The route had validated status since it was written; every stray came from a scratchpad script writing through the admin client, which goes around the route. A row on an unknown status is worse than a rejected write, which is the same lesson `priority: 999` taught. Priority is 1 (highest) to 3.

**`in_progress` is capped, and the cap is the point of it.** `settings.task_wip_limit` (default 3) is the most tasks that may be started at once; `POST /api/tasks` and the PATCH both refuse a start past it (`src/app/api/tasks/wip.ts`), counting at the moment of the write rather than trusting a list the browser loaded a minute ago. Editing a task already in progress is not a start and is never refused. The number is one person's capacity, not the usual kanban 5 — those are set from headcount, so 5 describes a team of four. A limit above the real capacity restrains nothing, and the strip fills with work that stopped being in progress weeks ago. It is in `settings` because it may change; a missing or unparseable value falls back to 3 rather than to unlimited. The status was carrying **1 row out of 334** before this, because setting it meant opening the drawer, changing a dropdown and saving — three acts of bookkeeping for a fact true for an afternoon. `/tasks` now starts and stops from the row in one press, pins the started ones in a "Working on" strip above the filters, and marks any untouched for 7 days.
...
| status | text | `open` (default), `in_progress`, `blocked`, `done`, enforced by `tasks_status_check`. `in_progress` is capped by `settings.task_wip_limit` |
...
| due_date | date | |
| created_at | timestamp | |
| updated_at | timestamp | Set by the route, never a trigger — this schema has none. The "Working on" strip counts the stale marker from it |
