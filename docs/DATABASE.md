...
| uses_msg | boolean | Does this kitchen season with MSG — micin, penyedap, kaldu bubuk (migration 123)? `false` = cooks without it, `true` = uses it, **NULL = nobody has ever asked that kitchen, which is not the same as no**. Homey is false; Thenie is true because they season with kaldu jamur merek Totole — not pure micin, but a flavour enhancer, which is how an admin answered the Lippo Karawaci lead on 2026-09-09 and where this fact came from. The brand is recorded here and deliberately never reaches the prompt, since a customer asking about MSG is asking what is in the food, not which supplier we buy from. Santapin is NULL and stays NULL until someone asks them. Read like `offers_size_m`: the prompt renders it from `dapurOptions[].usesMsg` by nickname, names the MSG-free kitchens, says plainly which ones use penyedap, and escalates with `ask_admin_for_help` for a NULL rather than guessing. The question had no answer anywhere in the prompt until 2026-09-19, and four customers asked it between 1 and 17 September |
