...
**Straightness is post-processed, not prompted.** The model copies the reference's tilt along with its portion, so five photos came back at five angles. The reference is now deskewed and frames the whole tray, and `deskew()` rotates every generated PNG upright off its own alpha silhouette before writing it — see "Fullness is not controllable in prose" in `docs/DESIGN_SYSTEM.md`.
