...
**The prompt describes the real box, and the real box is modest.** `scripts/assets/reference-box-2026-08-18.jpg` is the delivery photo it is written from — replace it and rewrite the prompt together when the packaging changes. Black glossy plastic, four moulded compartments, and visible bare tray around every item — the first version of this prompt said "white paper box, five compartments, filled edge to edge" and generated a heaped restaurant plate. A card that shows more food than the tray holds is the Batch 51 complaint in a worse form: Naya could be answered in words, an oversold photo cannot. Re-check the wording against a real delivery photo whenever the packaging changes.
