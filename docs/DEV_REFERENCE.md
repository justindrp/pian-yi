...
│   │   ├── catalog/ (kitchens.ts: `loadCatalog()` — the active kitchens as the catalog and landing page show them, nickname only, no id; plus the WhatsApp number, `chatLink()`, `BRAND` for what customers see, and `LEGAL_NAME`/`NIB` for the legal footer)
│   │   ├── subcontractors/
