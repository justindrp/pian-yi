...
**The balance is prepaid and manual — DeepSeek has no auto-debit, so it reaches zero on its own schedule.** Three outages so far: 24 Agustus (two hours, four customers unanswered), 5-6 September (overnight, two ad leads welcomed and abandoned) and 16 September 12:13 WIB, which was still down a day later because the top-up itself was erroring. `pnpm check-balance` reads it and exits 1 when it is under $2 or empty, 2 when it cannot be read at all; the endpoint is derived from `ANTHROPIC_BASE_URL`, so repointing the app at another provider makes the check fail loudly instead of reporting a balance nobody spends any more. It is watched from a Claude Code cron rather than a Railway one, because that is where Justin reads things — which also means the watch dies with the session and has to be recreated, and a Railway cron is the fix if that turns out to matter. An empty balance also disables the claim checker silently: `validateReply()` fails open on any error, so `scripts/reply-chat.ts` logs `validator call failed, failing open: 402 Insufficient Balance` and then `validator: ok`, one line apart. That is deliberate and stays — the only sender during an outage is a chat-review session writing claims straight off the ledger, so there is nothing for a smaller model with no database access to check.
...
**Where it goes.** The rendered system prompt is ~21K tokens (layer 4 above claims a 3000-token cap; that has not been true for months). A single inbound message can carry it three times — the reply, the validator retry, order extraction — plus the smaller `learn-context` and intent-classification calls. ~20K input tokens at the peak miss rate is ~$0.009 per message; at 55 messages/day that is ~$0.48, which is the observed figure. No price increase is needed to explain the bill.

**Where the cache breaks, and how to see it.** DeepSeek caches on exact prefix match, and `usage.input_tokens` on its Anthropic-compatible endpoint counts **cache misses only** — the hits are in `cache_read_input_tokens`. So a call reporting 38 input tokens against a 21K-token prompt is not a small prompt; it is a nearly perfect cache hit, and reading that field as the prompt size is how a caching problem hides. Measure the prompt by rendering it, never by the usage field.

Everything a request sends is serialised tools → system → messages, so the first byte that differs between two calls invalidates every byte after it — the tools included, if the difference is in them.

**The per-customer block is the last thing in the system prompt, and must stay there.** `buildSystemPrompt` (`src/lib/claude/prompts/system.ts`) collects everything that varies — the casual/polished mode sentence, the customer's own dapur, the size M offer for a customer already eating S, the dapur line on the order form, the daily-quota section, `## Current context`, the WIB clock, the schedule — into `perCustomerBlock`, appended after the last invariant rule. A rule that only *depends* on a per-customer fact does not have to move with it: the area gate is written once as constant text and points at the `Area customer ini` line in the tail, which keeps the rule in the cached prefix and the fact in the one place it varies. Measured 2026-09-08 before that change: 21,079 of the prompt's 21,332 tokens were identical for every customer, but the mode sentence sat at token 56 — so ~94% of every prompt billed at the miss rate, on the first call, on each tool round and on each validator retry, and the median chat call burned 7,089 uncached tokens. After: the shared prefix is ~20,900 tokens and the tail is 441–2,024. `test/api/system-prompt.test.ts` (`describe("the cacheable prefix")`) fails if a per-customer interpolation is put back into the body — everything below such a line stops caching too.
...
**The second model call on every message is gone.** `analyzeCustomerMessage` — the Assistant's own prompt plus all 24 of its tools, up to five turns — used to run on every inbound customer message from `processSavedCustomerMessage`. Over the fortnight to 2026-09-08 that was ~1,000 runs, which proposed a write action on 126 messages, and **94 of those 126 were `send_whatsapp_message` or `send_whatsapp_image`**: the Assistant offering to say something to a customer the bot had already answered seconds earlier. Nine high-priority pushes a day, three quarters of them duplicates. It now runs only on the escalated branch in `processWebhookAsync`, where the bot is silent and an admin is the only one who can act. The bot's own `ask_admin_for_help`, `flagOrderAtRisk` and the validator's `needs_human_review` flag are what cover the normal path. Its tokens are still not charged through `updateTokenCount`, so this spend appears in neither the per-customer rate limit nor the `/reports` cost table.
...
│   │   │   ├── chatbot-training/ (route for the system-prompt training chat)
...
│   │   │   ├── validate-reply.ts (the claim checker — Haiku, before send)
...
│   │   │   ├── chatbot-training-client.tsx (chat UI for crafting system-prompt instructions)
...
└── scripts/
    ├── check-balance.ts (re-runnable; prints the model account's USD balance and exits non-zero when it is low — 1 for under $2 or empty, 2 when the endpoint could not be read, so a cron can branch on it. `pnpm check-balance`, or `pnpm check-balance 5` for a different threshold. Read-only, and it touches no Supabase table. Written 2026-09-17, a day into the third outage, because nothing had ever watched the number that silences the whole bot)
    ...
    ├── fix-nakita-july-order.ts (one-off, 2026-09-01; writes the package Nakita paid for on 21 Juli and never got — 11 porsi x Rp 28.000 = Rp 308.000, agreed in the 20 Juli chat, transferred that night, matched by the BCA credit "NAQHITA SUHANDA" — and repoints her six 22-24 Juli delivery rows onto it. They had been charged to 0f573639, a May package of 12 porsi already holding 9 draws, taking it to 21 draws on 12 bought. The new order books 12 against 11 bought on purpose: the twelfth is the 1 porsi she was told she still had, and netting is a customer-level sum, never a per-order floor. Dry run by default, `--apply` to write; refuses unless exactly the six rows are still on the old order. Run: `npx tsx --env-file=.env.local scripts/fix-nakita-july-order.ts [--apply]`)
    ├── fix-kitchen-notes-audit.ts (one-off, 2026-09-01; swept all 416 customers' `kitchen_notes` against their own chat. Rewrote six — Naya, Vania and Winy were null while the preference sat only in the `[AI learned context]` block the kitchen sheet may not read; Julian and Sharleen carried day-specific instructions that had already expired; Rachel's standing "Rabu cumi" line was dropped after she was told it only runs on a Sabtu confirmation. Trimmed the migration 089 seed's trailing whitespace off fourteen more. Carolin was deliberately left null: her notes block claims "tidak pedas, tanpa seafood" and her chat contains no such request, and seeding the kitchen from that block is the exact bug migration 089 exists to stop. `--apply` to write; every change is in `edit_log` under `system:kitchen-notes-audit-2026-09-01`)
    ├── post-courier-august-journals.ts (one-off, 2026-09-01; the courier's whole August wage cycle as fifteen journals — ten cash advances, the 3.000.000 accrual, the two deductions, the advance offset, and the two transfers that settled it on 1 September. Written because 5002, 1201 and 1003 all had zero journal lines: the statements were imported and classified but nothing was ever posted, so the ledger showed a business with no delivery cost and a courier who had neither been advanced money nor paid. Reads the ten advances out of `bank_transactions` rather than retyping them, so an entry cannot disagree with its bank line, and sets `journal_id` on each so they leave the reconcile queue. Refuses to write unless every entry balances and both 1201 and 2001 net to nil. Scoped to one person's month, which leaves 1003 at -500.000 until the Superbank statements are journalised in full — the alternative was a settlement clearing a payable nobody accrued, which is worse, because an incomplete bank balance is obvious and a phantom payable is not. `--apply` to write)
    ├── import-superbank-august-screenshots.ts (one-off, 2026-09-01; the 24 Superbank rows for 1-17 Agustus 2026, typed out of the three app screenshots Agnes sent on 17 Agustus. Superbank will never issue an August statement — she resigned that day and swept the balance to BCA — so this is the only record the month has. A screenshot set is normally untrustworthy because you cannot see what scrolled past, but this one is provable: July closed at 673.009,37 and 673.009 + 6.000.000 - 5.681.000 is exactly the 992.009 that arrives in BCA the same morning, so the chain ties to the sen and the capture is complete. `balance_after` is left null on every row rather than back-computed from that chain — the app does not print it, and a derived figure in a column that means "what the bank said" stops being evidence. August credits face 1005 ShopeePay, not the 1002 July used: BCA has no direct transfer to Superbank on those four dates, it has a 1.500.000 ShopeePay debit on each. One row imported without a contra account — a masked Rp 15.000 payee on 10 Agustus — turned out to be the Lalamove ride bought to collect the food and take it to the right address after the courier delivered to the wrong one, so all 24 rows are classified. `--apply` to write; refuses to run twice)
    ...
    ├── review-deliveries.ts (re-runnable; one kitchen's delivery sheet for a date printed next to each customer's recent chat, so the two can be compared line by line. `pnpm review-deliveries` takes tomorrow and Dapur Suplir; `--date YYYY-MM-DD`, `--kitchen <nickname|name|id>`, `--messages N` (default 25). Rooted in `daily_deliveries` and filtered on the **row's** `subcontractor_id`, not the order's, because a package may be split across kitchens. Two sections: every row with its customer's `kitchen_notes`, address slot and drawn order, then the customers of that kitchen holding an active order who wrote to us in the last 48h and have **no** row for the date — a skip that was asked for and never applied is invisible on the sheet, because it looks exactly like a day nobody booked, and only the chat separates the two. Written 2026-09-13 for the 16:00 job: the order cutoff has just passed, tomorrow's sheet is final, and that is the last moment a wrong row can be fixed before the kitchen cooks it. Not `audit-sheet-data.ts`, which audits the 2025 Google Sheets import. Run: `pnpm review-deliveries`)
    ├── review-chats.ts (re-runnable; the batch read behind a chat review — prints whole threads with the customer row, `customer_flags`, `customer_state`, the last five orders with their quota counted off `daily_deliveries`, and the transcript in WIB naming the author of every outbound line. Four ways to pick the threads: `pnpm review-chats` takes everyone with an inbound in the last 24h (`--since 6h`, or an ISO timestamp), `pnpm review-chats --last 2` the most recently active threads, `pnpm review-chats --phone +62...` one, and `pnpm review-chats --waiting` every thread carrying `needs_human_review`, `pending_bot_response` or `escalated_to_human` that is still warm (last message within 3 days, `--days N` to widen). `--waiting` exists because a window selector cannot see a standing debt: the +6281212021234 lead's last inbound was 18.30 the previous evening, so a two-hourly `--since 2h` review would never have shown the thread it most needed to. Unfiltered the flags return 38 threads, most of them settled months ago and never cleared, which is why the warmth cutoff is not optional. `--messages N` sets transcript depth, default 60. The 24h selector reads `review_inbound_threads` (migration 119), one row per customer carrying their newest inbound, so "had an inbound since X" is a filter on that timestamp and the distinct runs in Postgres; it used to page every inbound row in the window 1000 at a time and dedupe in JavaScript, which was 4441 rows fetched to derive 328 ids over all time. It cannot key off `inbox_threads` instead — that is the last message per customer whatever its role, and the 08:00 keep-alive cron writes outbound to ~10 threads at once, which would pull every pinged thread into a review the customer never answered. Threads are fetched through a six-wide pool and printed in the order `selectThreads` chose, so the read stays newest-first while the round-trips overlap; `renderThread` returns its block rather than printing it, because concurrent `console.log` interleaves threads. The `subcontractors` map is read once in `main` and passed down, and one `daily_deliveries` query covers all five orders — both were a round-trip per thread, which is what made a 2026-09-17 run of 11 threads take 6.3s where the same run now takes 2.2s. The per-order quota line counts **portions, not rows** — `package_size` is portions and a single `daily_deliveries` row may carry several, so summing rows reported Ireine's 20-portion order as "booked 10, delivered 0, unbooked 10" when every portion was already on the calendar (fixed 2026-09-13). Written on 2026-09-12 because every review before it was a throwaway script: each one picked threads differently, and none printed the flags that decide whether anyone is actually answering the thread — the +6281212021234 lead sat nine hours on "Bentar ya kak, aku cek dulu sama admin" with `escalated_to_human` false, which no transcript dump would have shown. Not `watch-thread.ts`, which is one phone and a shorter dump. Run: `pnpm review-chats`)
    ├── review-leads.ts (re-runnable; the pipeline read. `pnpm review-leads` prints everyone with an inbound in the last 3 days who has **never had an order paid** (`--days N` to widen), newest inbound first, with the area, whether a Maps link is on file, the kitchen if one has been picked, the 24h window state, any `pending_payment` order, and the first and last message of the window naming the author. `--transcripts` prints every message of every lead thread instead of that pair — why a lead went quiet is only readable in the whole thread, and a first/last pair cannot tell a price objection from a question nobody answered; asking it once here is what stops a fourth throwaway script being written to read the same rows. A customer holding a `pending_payment` order is still listed: the money has not arrived, so the lead is not won. Then every `event_leads` row, open or closed, with its H-countdown and what was quoted — a one-off event never becomes an `orders` row until somebody writes one by hand, so it is invisible to the first list by design. Written 2026-09-18 after the third throwaway `leads.ts`; the earlier ones each selected differently and one of them returned only the oldest conversations, which made its newest leads look unreachable — every read here walks with `fetchAllRows()`. Named for the `review-` prefix the other two batch reads carry, so tab-completion surfaces the three of them together. Run: `pnpm review-leads`)
    ...
    ├── watch-inbound.ts (long-running; prints one line per inbound message on one customer's thread and never exits. For a thread held off the bot with no auto-resume — `customer_flags.last_human_activity_at` null, which `shouldAutoResume()` never hands back. A hold like that with nothing watching it is the Sherine Fayola failure: the model is skipped, the message lands in `conversations`, and nobody is told. stdout is the whole interface, one line per message, so a person can tail it or a monitor can turn each line into a notification. Polls every 30s. A poll that fails is retried once immediately, because a socket kept alive across the 30s idle is dead by the time the next poll reuses it and undici reports that as `TypeError: fetch failed` without retrying — the watcher ran for hours on 2026-09-11 failing every poll while `curl` to the same host answered in 2ms, which looks exactly like a quiet thread. Ten failed polls in a row now kill the process rather than log and loop, because a blind watcher is the hold-with-no-watcher failure it was written to prevent. Not `watch-thread.ts`, which is a one-shot dump. Run: `pnpm tsx --env-file=.env.local scripts/watch-inbound.ts +62...`)
    ...
    ├── sim-pricing.ts (re-runnable; sends real messages to the chatbot on `DEMO_` phones and checks the prices it quotes back — six cases: the 5-porsi floor, an off-list 15, a 7 that divides by neither, 22 box × 5 hari, a skipped day that must not shrink the package, and one whole order written by `extract_order`. It exists because migration 098 keyed `pricing_tiers` by kitchen and a read that forgets the kitchen does not throw — it quotes another kitchen's ladder and looks like it worked. Unit tests mock the table away, so the only way to see the scope is to ask the bot. Each case's demo customer and everything it created is deleted at the end (`--keep` to inspect one). Run: `pnpm tsx --env-file=.env.local scripts/sim-pricing.ts [--only=<case-id>] [--keep]`)
    ...
    ├── invoice.ts (re-runnable; renders one A4 invoice PDF from a JSON spec by calling `renderInvoicePdf()` in `src/lib/invoices/render.ts` — the same renderer the bot's `send_invoice` tool uses, so a hand-run invoice and a bot-sent one cannot drift into two layouts. It was headless chromium until 2026-08-31; chromium is a dev-only dependency and Railway has no browser, so the layout moved to pdfkit when the bot needed it. It embeds Plus Jakarta Sans from `src/lib/brand/fonts/`, which `next.config.ts` traces into the standalone build — drop that include and every production send throws on a missing font file. The spec also draws a **quotation** (`"kind": "quotation"` retitles it PENAWARAN, turns *Jatuh tempo* into *Berlaku s/d* and drops the balance line) and takes an optional `brand.lines` header (the lockup itself is drawn from `LOCKUP` in `src/lib/brand/logo.ts`; the default lines end with the "dulu Pian Yi Catering" bridge) — a company paying for an event asks for a quotation before it will transfer, and the first one (Natalie Ryan, 25 Sep 2026) went out as Katerloka. Renders only: nothing is uploaded, nothing is sent, and no `invoices` row is written — for a real send use the bot's tool or `POST /api/inbox/manual-document`. Run: `npx tsx scripts/invoice.ts spec.json out.pdf`)
...
`menu_text` has to be in the shape `parseMenu()` reads before any of this works — a kitchen's intake message pasted in as sent renders an empty card (see "The weekly menu card" in `docs/DESIGN_SYSTEM.md`). `menu-photos.ts` generates one food photo per menu day with OpenAI `gpt-image-2` (`OPENAI_API_KEY`, local only — nothing on Railway calls it). `menu-card.ts` renders 1080×1350 at 2x through headless chromium and writes `.menu-photos/card-<nickname-slug>.png` (gitignored).

**Both scripts take `--kitchen` and both key their files on the kitchen.** Every kitchen cooks a different week, so `--kitchen` matches an id, a nickname or a name and defaults to the first active kitchen with a `menu_text`; without it the scripts only ever saw one kitchen. `menu-photos.ts` had the defaulting but never the flag until 2026-09-13, so it always drew whichever kitchen sorted first — by then that was Dapur Palem, and a Batch 54 run for Dapur Suplir would have billed five photos of somebody else's week and written them into Suplir's folder under Suplir's dish names. The matcher is now the same code in both. Photos live in `.menu-photos/<nickname-slug>/tN.png`, not flat at the top of the directory — while they were flat, Dapur Monstera's first card printed its own dish names under Dapur Suplir's food photos, which is precisely the failure the pipeline exists to prevent. A kitchen may be named even when it is inactive, so its card can be rendered before it is switched on.

**Nothing on the card is typed.** The batch, the dates, the dishes and the size M item are parsed out of `subcontractors.menu_text` for the kitchen being rendered; the surcharge is that kitchen's own `size_m_surcharge`, house setting when NULL; the delivery days are that kitchen's `delivery_days` and its areas its own `delivery_areas`, falling back to `activeDeliveryAreas()` only when the row carries none. Both scripts parse that column the same way, so the photo prompt and the printed dish names cannot drift apart — which is the whole point. Batch 51's card was drawn by hand in a chat window and three of its five photos showed food nobody cooks that week; Senin's "Chicken Katsu" was plated as tempeh sticks. Next week is a re-run, not a redraw.
...
**The card is the Katerloka sheet** — a flat nasi ground (`#F7F5F1`, so the corner pixel samples `srgb(247,245,241)`; `magick .menu-photos/card-<dapur>.png -format "%[pixel:p{4,4}]" info:` is the check), white day cards, a daun footer. Colours come from `BRAND` in `src/lib/brand/logo.ts`, never a literal in the script; type is Plus Jakarta Sans, fetched from Google Fonts at render time (no system install). The rules are in `docs/DESIGN_SYSTEM.md`.
...
```
pnpm tsx --env-file=.env.local scripts/price-list.ts [--all] [--kitchen X] [--upload]
```
...
**One sheet per kitchen, because the customer picks their kitchen and is shown that kitchen's prices.** Migration 098 gave each kitchen its own ladder and 099 its own `price_list_image_url`; a single sheet was correct for Thenie and wrong by up to Rp 16.000 a portion for the rest. Everything on a sheet is that kitchen's own: its ladder (`tiersForKitchen()`), its `delivery_areas` rather than the union, its `offers_size_m`, its `delivery_days` — which is where the "Senin–Sabtu" subtitle comes from, so Dapur Monstera's sheet reads Senin–Jumat rather than advertising a day nobody cooks. There is no house sheet any more: run with no kitchen to render and the script stops.

`--all` renders inactive kitchens too: a sheet has to be looked at before its kitchen goes live, and the kitchen is not active until it has been. `--kitchen` narrows that to one, the same spelling `menu-card.ts` takes — **use it to publish a prospect's sheet.** `--all --upload` writes a `price_list_image_url` to all fifteen rows, and eleven of them are prospects with no `pricing_tiers` of their own — a kitchen with no ladder cannot be rendered, and cannot be activated either (migration 135). `--upload` publishes each PNG to the `menu` bucket and writes its URL to that kitchen's `price_list_image_url`.

**Nothing on it is typed.** The rates come from `pricing_tiers`, the size M surcharge from that kitchen's own `size_m_surcharge` (house setting when NULL, migration 131 — one sheet cannot carry two kitchens' M prices any more than it can carry two ladders), the areas and days from the kitchen's own row, and the ongkir line from that kitchen's `subcontractor_neighborhoods` fees measured against the kecamatan of its own `delivery_areas` (see "The price list" in `docs/DESIGN_SYSTEM.md` — it used to be the fixed words "Gratis ongkir"). A tier change is a re-run, not a redraw. An S-only sheet pads its rows to fill the page: an M cell carries two prices and an S cell one, so without that the footer's `margin-top:auto` parks it at the bottom over an empty band — and every kitchen but Thenie is S only, so that is the normal case now. Layout rules and the reasons behind them: "The price list" in `docs/DESIGN_SYSTEM.md`.

**A render nobody uploads changes nothing.** `--upload` does it; by hand, the sheet reaches customers through `subcontractors.price_list_image_url`, which `send_price_list` and the welcome read — upload the PNG to the `menu` bucket and point the kitchen's column at the new object. There is no fallback: an active kitchen must have one (migration 134), so render and upload it before switching the kitchen on. `settings.price_list_image_url` is only the house sheet shown on the public landing page. Nothing deletes the old one, so an image already sent keeps resolving, and the flip is a settings write like any other: audit it with `logEdit()` and let the 60-second settings-cache refresh pick it up. Until 2026-08-31 that setting pointed at `price-list-personal-s-1787384421668.png`, a hand-drawn S-only sheet, which is why a customer asking for prices never learned size M existed; it now points at `price-list-personal-1788170708513.png`, this script's output.
...
- A missed job is retried by a **sweep that runs every 10 minutes**, not only at boot. `catchUpMissedJobs()` compares each `catchUp` job's `cron_runs` row against its last due time and re-runs whatever is behind, under the same-day rule; only a *successful* run is recorded, so a failure stays visible to the next sweep. Until 2026-09-21 that sweep fired at boot and nowhere else, which meant a job that failed while the app stayed up waited for the next deploy — and a database outage does not restart the app. That day every cron failed against a wedged PostgREST while Railway sat healthy, which would have left `cancel-unpaid` unrun straight through its 16:00 deadline. A job is retried at most 5 times per Jakarta day, because every `catchUp` job tolerates a redundant run but not forty of them, and the two sweeps cannot overlap — both call `runJob` directly, which is the one path Croner's `protect` does not cover
...
- `expire-pending-questions` (hourly at :40) clears `customer_flags.pending_bot_response` on a question the customer has stopped chasing. The flag is set by `ask_admin_for_help` and by the webhook's claimed-escalation guard, and nothing but a human ever unset it, so a question no admin answered stayed live forever: on 2026-09-09, 19 of the 25 threads on the Inbox `Unanswered` tab were dead flags — one since 3 July with no question text at all. A tab reading three times the real backlog is a tab nobody reads, which is how the six live threads sat for days. The job dates the flag from the customer's **last message**, not from when we parked it — the webhook restamps `pending_bot_question_at` on every inbound while the flag stands — so a thread that keeps asking never ages out, where a flat age cutoff would have dropped a lead genuinely still waiting on day five. It messages nobody: clearing the flag just lets the model answer normally again. Window in `settings.pending_question_expiry_hours` (48); the question text goes to `edit_log` before it is destroyed. See "customer_flags" in `DATABASE.md`
