...
└── scripts/
    ...
    ├── invoice.ts (re-runnable; renders one A4 invoice PDF from a JSON spec through headless chromium — number, dates, bill-to/ship-to, line items with an optional sub-line, subtotal, ongkir, total, a LUNAS stamp, the payment block and footer notes. It exists because the first invoice sent to a customer was drawn by a throwaway script that was deleted the same day, so the correction had to be rebuilt from a screenshot of the PDF. Renders only: nothing is uploaded and nothing is sent — hand the PDF to `scripts/manual-send-image.ts`'s document sibling or `POST /api/inbox/manual-document`. Run: `npx tsx scripts/invoice.ts spec.json out.pdf`)
