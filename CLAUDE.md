...
- **Size M is per kitchen** (`subcontractors.offers_size_m`), never a hardcoded kitchen: S price plus `settings.size_m_surcharge` (Rp 4.000/porsi) on every tier and on a contract rate. A kitchen that does not cook M has an M order written as S rather than refused. See "Order sizes (S / M)" in `docs/OPERATIONS.md`.
