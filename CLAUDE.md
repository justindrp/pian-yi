...
- **Fixed 2026-07-06:** extract-order modal now supports per-day varying portions via a "Per hari" toggle. When enabled, admin specifies a `delivery_schedule[]` (date + meal_type + portions per slot). The order is created as `order_type: "scheduled"` with `daily_deliveries` rows pre-inserted immediately. Package size is auto-summed from the schedule; pricing updates live. The `extract_order` tool schema also accepts `delivery_schedule` so Claude can auto-populate it when the chat shows varying portions per day.
- **Fixed 2026-07-06:** delivery proof sends now use the `delivery_proof` WhatsApp template (image header, no body variables) via `sendImageTemplate` in `client.ts`, replacing the plain image message that failed outside the 24h conversation window. `sendDeliveryPhotoToCustomer` falls back to a direct `customers` lookup when the customer has no `daily_deliveries` row today (common for manual inbox resends).
...
- **Fixed 2026-07-05:** Legacy-imported customers (created directly in the DB with an active order, bypassing the webhook) never got a `customer_state` row with `menu_shown = true`. Their first real WhatsApp message therefore triggered the full new-customer welcome sequence (menu images, price list) and returned early — skipping Claude entirely for that turn, so the "returning customer" prompt rule never ran. Root cause of Ari Budiyanti seeing a fresh onboarding flow despite an active order, and the bot then fabricating "data pesanan lama tidak tersimpan" when she pushed back. Welcome-sequence gate (`src/app/api/webhook/whatsapp/route.ts`) now also checks `latestOrderStatus`; any customer with an existing order skips onboarding and goes straight to Claude.
