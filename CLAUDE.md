...
- **A number in `customer_contacts` is a delivery recipient, not a customer** (migration 118): the webhook answers it with `handleProofContactMessage()` — its own prompt and two tools, `send_delivery_proof` aimed at the owner's id plus `ask_admin_for_help`. Never route one through the ordering pipeline: it carries the ladder, the quota and the payment flow, and a recipient is not the buyer. The link lapses the moment that number holds an order of its own. See `docs/BOT_RULES.md`.
