...
**Confidentiality.** Never disclose **any** subcontractor's real name to customers, in any form. Customers only ever see the `customer_nickname`, which since 2026-09-04 is an ornamental-plant name ("Dapur Suplir") rather than a number — read it from the row, never write one into a prompt or a doc. The rule covers every row in `subcontractors`, present and future — never an enumerated list, in a doc or a prompt, because such a list is always short by however many kitchens were added since it was written. Frame as "dapur partner kami". What is confidential is *which* kitchens, never that partner kitchens exist: a customer who names a supplier gets neither denial nor confirmation, because "kami masak sendiri" is a lie they can find out. **Never put the bank account number in the bot's system prompt** — the payment message is composed by `createOrderFromExtraction`, and the model has never needed it. Never reveal COGS, margins, or internal operations; customer-facing errors are always generic.
...
**Pricing.** The ladder in `pricing_tiers`, 5 → 144 portions, 29k → 25k per portion for the house; **nothing mirrors it in code any more** — the prompt's price list is rendered from the rows by `priceListLines()`, per kitchen, and every quote path in `extract-order.ts` takes a `subcontractorId`. **It is one ladder per kitchen** (migration 098): `subcontractor_id IS NULL` is the house ladder, equal to Thenie's, and every read goes through `tiersForKitchen()` (`src/lib/pricing/tiers.ts`) — a bare select returns every kitchen's rows interleaved and quotes whichever sorts first, and a write keyed on `portions` alone reprices every kitchen at once. A total off the list but divisible by 5 or 6 is sellable at the rate of the **largest listed size below it** — never as repeated smaller packages, which charged more for a bigger order. **A total that is neither is refused in code** — `createOrderFromExtraction` withholds the order and offers the nearest two sizes, because the bot sold a 7-porsi package that does not exist. `customers.contract_price_per_portion` replaces the ladder entirely for corporate customers. **The customer picks their kitchen; we never assign one and it never follows their area** — several kitchens cover most areas, so the area only narrows the list. A returning customer's kitchen is on `customers.subcontractor_id`, and it must reach the prompt as `currentDapur` or the bot asks them which dapur they are on and then invents a rule to explain it (2026-09-06: Veronica Catherine, on Thenie since June, was quoted Thenie's Rp 145.000 and billed Santapin's Rp 152.500). See "The customer's own dapur belongs in the prompt" in `docs/BOT_RULES.md`. **Size M is per kitchen (`subcontractors.offers_size_m`), never a hardcoded kitchen** — it costs the S price plus `settings.size_m_surcharge` (Rp 4.000/porsi) on every tier and on a contract rate, and a kitchen that does not cook it has an M order written as S rather than refused. See "Order sizes (S / M)" in `docs/OPERATIONS.md`. Existing orders lock `price_per_portion` at creation. **A one-off event order is priced by tendering it to the kitchens, never off the ladder** — the bot gathers the brief and escalates, and must not call `extract_order`, because creating the order is what sends the bank details. See "A custom/event order is tendered to the kitchens" in `docs/OPERATIONS.md`.
...
**Delivery.** **Which days are worked is per kitchen, and that includes Minggu** (`subcontractors.delivery_days`, ISO weekday numbers): Santapin cooks seven days, Homey five, Thenie six. Pass the kitchen's list — `isDeliveryDay(ymd, days)` (`src/lib/holidays/id.ts`) and `kitchenDeliversOn()` (`src/lib/subcontractors/days.ts`) — or a Mon–Fri kitchen gets a Sabtu row nobody cooks and a seven-day kitchen is refused a Minggu it would have delivered. `BUSINESS_DAYS` (Senin–Sabtu) is the fallback for a path that cannot name a kitchen, never a fact about the business; a page or a prompt that needs the whole picture reads `activeDeliveryDays(db)`. Libur nasional closed for every kitchen (`src/lib/holidays/id.ts` — extend before it lapses on 2026-12-31), **except the dates in `OPEN_DESPITE_HOLIDAY`, which the active kitchens work through** — always ask `isClosedHoliday()`, never `holiday.type`, or the prompt and the generator disagree and a customer is told no about food already on the sheet. Cuti bersama is an escalation, not a closure. Order deadline 16:00 WIB the day before (`settings.order_deadline_hour`), for changes and skips as well as new orders. The bot is handed the WIB clock and a computed "cutoff passed / still open" verdict plus the soonest deliverable date (`src/lib/time/jakarta.ts`) — never the bare hour, which it read as permanently still ahead. Areas come from active subcontractors' `delivery_areas`, never `settings` and never a literal in code, a prompt or a doc — read them with `activeDeliveryAreas(db)` (`src/lib/subcontractors/areas.ts`) or `useDeliveryAreas()` in the dashboard. They are **per kitchen, not global**, and some areas rest on a single kitchen, so deactivating one subcontractor can remove an area entirely.

**The kitchen cooks from `customers.kitchen_notes` and nothing else** (migration 089) — written by an admin or by `extract_order`'s `catatan`, never by the summarizer. `/dapur/[id]` used to print the `Preferensi:` bullet out of the `[AI learned context]` block in `customers.notes`, which `learnCustomerContext()` rewrites wholesale, so a hand correction lasted until the customer's next message: on 2026-09-01 Carolin was cooked six portions tanpa nasi off a restriction she never gave. Never route a model summary to that page. See `docs/BOT_RULES.md`.
...
**A delivery row means one thing: this food will be cooked and delivered. Present or absent, nothing in between** (migration 075). `daily_deliveries.status` and `orders.portions_remaining` are both gone. **Skipping is a `DELETE`**, never a status change — `deleteDelivery()` (`src/lib/orders/delivery-state.ts`) copies the whole row into `edit_log` first, because nothing else can rebuild it. Nothing else is written on a skip: the balance is `package_size` minus the rows that exist, so removing the row **is** the refund. Never "return the portions" to a counter, and never add a status column back — 2937 of 2937 rows only ever said `scheduled` while seven read paths each had to remember to exclude the values nothing wrote. Whether a delivery has happened is read off its date, never stored: `date <= today` is drawn, and `isLocked()` (past D-1 16:00) is when the kitchen is booked and the skip window has closed. The bot deletes rows through `delete_deliveries` (`src/lib/orders/delete-deliveries.ts`) and it, like the Assistant, may delete a row only for a date whose cutoff has not passed, **and a locked date cannot be re-addressed either** — the prompt marks each scheduled date TERKUNCI from `isLocked()` rather than leaving the model to work it out.
