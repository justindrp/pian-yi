...
- The only active subcontractor now delivers Saturday, so fixed-schedule orders are available 5 days (Senin–Jumat) or 6 days (Senin–Sabtu). The 6-day price rows on the public list are sellable again. Minggu is still closed, as are national holidays.
- Splitting a fixed-schedule package's deliveries across two addresses on different days is operationally supported via the per-day address override on the daily sheet. 5 hari + 1 extra day is now simply a 6-hari package. There is still no single-portion one-off order — bebas/quota packages only sell in fixed sizes (5, 10, 20, 40, 60, 120 portions), so an extra delivery on top of a package must draw from one of those.
- Custom fixed-schedule day counts that are multiples of 5 or of 6 use repeated blocks of that package. Examples: 15 days lunch-only = 3 × Rp 145k = Rp 435k; 12 days lunch-only = 2 × Rp 174k = Rp 348k. Counts that are neither must be rejected politely; tell customers to choose a multiple of 5 or 6.
- Bulk adjust supported: `PATCH /api/settings/pricing` with `{ adjust: number }` increments all tiers at once
- Bebas/quota package sizes offered by the chatbot: 5, 6, 10, 12, 20, 24, 40, 48, 60, 72, 120, 144 total portions. The 6-multiples were added alongside Saturday delivery; the prices are spelled out in the "Bebas/quota" section of `system.ts` and stored in `pricing_tiers`. Sizes not on that list must be refused, not rounded down to a tier.
