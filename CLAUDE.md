...
- **Size M is per kitchen, and so is its price** (`subcontractors.offers_size_m`, `subcontractors.size_m_surcharge`), never a hardcoded kitchen: S price plus that kitchen's own tambahan on every tier and on a contract rate, falling back to `settings.size_m_surcharge` (Rp 4.000/porsi) only where the column is NULL — the global figure is Thenie's, and Molls' is 6.500. Resolve it with `kitchenMSurcharge()` (`src/lib/pricing/tiers.ts`). A kitchen that does not cook M has an M order written as S rather than refused. See "Order sizes (S / M)" in `docs/OPERATIONS.md`.
