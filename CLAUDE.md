...
- No draw path checks the balance before writing, so a fully-used order can still go negative — 89 active orders currently read `portions_remaining` ≤ 0. A hard reject belongs in daily-sheet `PUT`, but only **after** the stale counters are reconciled; adding it first would block the daily sheet for customers whose balance is merely mis-recorded.
