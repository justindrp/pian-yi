...
- Inbound WhatsApp media expires. The webhook stores only Meta's `media_id`, and `/api/inbox/media/[mediaId]` resolves it live on every view — but Meta deletes inbound media after roughly a week, after which the thread shows a permanent broken image. `scripts/backfill-chat-media.ts` rescued the 15 rows that were still resolvable on 2026-08-12 into the private `chat-media` bucket and rewrote `conversations.content` to the stored URL; 60 rows were already unrecoverable. `getInboxImageSrc` / `getInboxDocument` therefore prefer a stored `/chat-media/` URL over `media_id`. **The webhook still does not save bytes at receipt time, so new images keep expiring** — fixing that is the durable half of this and is still open.
