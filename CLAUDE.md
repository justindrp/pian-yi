...
- `18:10 +0700` Fixed: bot kept asking Q0 (jadwal tetap vs pesan bebas) to returning customers on their first message, even though their ordering model was already knowable. Root cause: Q0's skip condition only checked whether the customer's own message made the model obvious — it never looked at customer history, so the separate "Returning vs new customers" rule (a soft hedge, "don't assume they need the full Q0→form walkthrough") never actually suppressed Q0. Fix (`system.ts`): Q0 now also skips when the customer has an active quota-based order (`activeOrder` present → model is already bebas/quota) or is a known returning customer whose notes/history show a prior model; the returning-customer rule now states a hard "skip Q0" instead of a hedge.
- `17:30 +0700` Changed: bebas/quota package sizes in `system.ts` no longer offer 72 or 144 total portions — sellable sizes now 5, 10, 20, 40, 60, 120 only (both the pricing rule and the "Yang tersedia" prompt text updated). `PRICE_LIST_LINES`' 72-hari fixed-schedule rows are unrelated (day-count pricing, not bebas packages) and untouched.
- `17:22 +0700` Changed: fixed-schedule ordering (`system.ts`) now asks days/meal-preference/portions-per-delivery/kitchen as one combined message instead of 4 separate one-at-a-time questions, to cut round-trips in a WA chat. Bot still asks again only for whatever the customer didn't answer.
- `17:00 +0700` Fixed: webhook idempotency check (`processed_messages`) was racy — it did a `select` for an existing row, then a separate `insert` if none was found, with the insert's result never checked. When Meta redelivers the same webhook event within milliseconds (a normal retry, not a bug on Meta's side), two concurrent requests could both pass the `select` before either `insert` landed, so both proceeded to call Sonnet and both sent a reply — reproduced live (two near-duplicate assistant messages 1.4s apart, both re-explaining reschedule policy, for one customer message "Buat siang aja kak"). `message_id` is already the table's primary key, so the second `insert` was silently failing on a unique-violation the whole time; code just never looked at the error. Fix: check the `insert`'s error and return early on any error (treat as "another request already claimed this message_id") — the insert is now the atomic guard, the earlier `select` is just a cheap pre-check.
