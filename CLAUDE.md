...
- **One ladder per kitchen** (migration 098). `subcontractor_id IS NULL` is the house ladder, equal to Thenie's. Every read goes through `tiersForKitchen()` (`src/lib/pricing/tiers.ts`): a bare select returns every kitchen's rows interleaved and quotes whichever sorts first, and a write keyed on `portions` alone reprices every kitchen at once.
