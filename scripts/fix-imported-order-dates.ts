...
async function main() {
  ...
  const multiPurchase = [...purchasesByName.entries()].filter(
    ([, p]) => p.length > 1,
  );
  ...
  for (const [sheetName, purchases] of multiPurchase) {
    ...
    const importOrders = orders.filter(
      (o) => o.created_at.startsWith("2026-06-08") && !isJunk(o),
    );
    const junkOrders = orders.filter((o) => isJunk(o));
    // In-app orders: created Jun 29 onward (before that = import artifacts)
    const inappOrders = orders.filter(
      (o) => o.created_at >= "2026-06-29" && !isJunk(o),
    );
    ...
    if (importOrders.length === 0) {
      if (inappOrders.length > 0) {
        skipped.push(
          `${sheetName}: no import order — has ${inappOrders.length} in-app order(s), skipping`,
        );
      } else {
    ...
    if (inappOrders.length > 0) {
      skipped.push(
        `${sheetName}: has ${inappOrders.length} in-app order(s) alongside import order — manual review needed`,
      );
      continue;
    }
    ...
    if (importOrders.length > 1) {
      skipped.push(
        `${sheetName}: ${importOrders.length} import orders — complex, manual review needed`,
      );
      continue;
    }
    ...
    if (
      existingOrder.portions_remaining > lastPurchase.porsi &&
      existingOrder.portions_remaining > 0
    ) {
      skipped.push(
    ...
    if (lastPurchase.price > 0 && lastPurchase.price < 5000) {
      skipped.push(
        `${sheetName}: suspicious price ${lastPurchase.price}/p — skip`,
      );
      continue;
    }
    // Also check any older purchase
    const suspiciousOlder = olderPurchases.find(
      (p) => p.price > 0 && p.price < 5000,
    );
    if (suspiciousOlder) {
      skipped.push(
        `${sheetName}: suspicious price ${suspiciousOlder.price}/p in ${suspiciousOlder.date} purchase — skip`,
      );
      continue;
    }
    ...
    console.log(`\n─── ${sheetName} ───`);
    console.log(
      `  Import order: ${existingOrder.id} (${existingOrder.start_date}, ${existingOrder.package_size}p @ ${existingOrder.price_per_portion}/p, remaining=${existingOrder.portions_remaining})`,
    );
    ...
    for (const junk of junkOrders) {
      ...
      if ((count ?? 0) > 0) {
        console.log(
          `  ⚠ Junk order ${junk.id} has ${count} deliveries — not deleting`,
        );
        continue;
      }
      console.log(
        `  🗑 Delete junk order ${junk.id} (${junk.start_date}, ${junk.package_size}p)`,
      );
      if (!dryRun) {
    ...
    for (const p of olderPurchases) {
      console.log(
        `  + Create completed order: ${p.date}, ${p.porsi}p @ ${p.price}/p = Rp${p.total.toLocaleString("id-ID")}`,
      );
      if (!dryRun) {
        const { error } = await db.from("orders").insert({
          ...
          meal_time_preference:
            existingOrder.meal_time_preference || "per_day_decision",
    ...
    console.log(
      `  ✎ Update import order → ${lastPurchase.date}, ${lastPurchase.porsi}p @ ${lastPurchase.price}/p = Rp${lastPurchase.total.toLocaleString("id-ID")} (keeping portions_remaining=${existingOrder.portions_remaining})`,
    );
    if (!dryRun) {
