...
async function main() {
  ...
  w("");
  w(
    `_Generated ${today}. Re-run: \`set -a && . ./.env.local && set +a && pnpm tsx scripts/audit-sheet-data.ts\`_`,
  );
  w("");
  w(
    `CUSTOMERS sheet rows: **${dbNames.length}** · package_orders rows: **${pkgRows.length}** · ORDER_HARIAN rows: **${harRows.length}**`,
  );
  w("");
  ...
  for (const r of pkgRows) {
    ...
    if (!nama || nama === "#N/A") {
      ...
      if (parseDate(tgl) || porsi > 0 || total > 0) {
        pkgOrphan.push(
          `${tgl || "no date"} · ${porsi} porsi · Rp${total.toLocaleString("id-ID")}`,
        );
      } else {
        pkgFiller++;
      }
    ...
    if (porsi > 500)
      pkgSuspicious.push(
        `${nama}: porsi=${porsi} (${col(r, ["tanggal", "date"])})`,
      );
  ...
  if (pkgUnmatched.size === 0) {
    ...
    w("|---|---|---|---|");
    for (const [name, a] of [...pkgUnmatched].sort(
      (x, y) => y[1].porsi - x[1].porsi,
    )) {
  ...
  if (harUnmatched.size === 0) {
    ...
    w("|---|---|---|---|");
    for (const [name, a] of [...harUnmatched].sort(
      (x, y) => y[1].porsi - x[1].porsi,
    )) {
      const s = suggest(name, dbNames);
  ...
  w("");
  w(
    `Remaining = Σ package_orders porsi − Σ ORDER_HARIAN porsi delivered on or before ${today}, matched by CUSTOMERS sheet name. Names from A/C (unmatched rows) are excluded — fix those first, they are not counted here.`,
  );
  w("");
  ...
  w(
    `**${realDeficit.length} customer(s) with negative computed remaining despite having logged purchases** (candidates for free/goodwill quota — see Section F):`,
  );
  w("");
  if (realDeficit.length === 0) {
    ...
    w("|---|---|---|---|");
    for (const r of realDeficit)
      w(`| ${r.name} | ${r.purchased} | ${r.delivered} | ${r.remaining} |`);
  }
  w("");
  w(
    `**${noHistory.length} customer(s) with 0 logged purchases but deliveries exist** — package_orders was only backfilled from Dec 1, 2025 onward; these predate the backfill and are not a real deficit:`,
  );
  w("");
  ...
  w("| Customer | Purchased | Delivered | Remaining |");
  w("|---|---|---|---|");
  for (const r of [...remainingRows].sort((a, b) =>
  ...
  if (deficitNames.length === 0) {
    ...
    for (const name of deficitNames) {
      const timeline = [
        ...(pkgEvents.get(name) ?? []).map((e) => ({
          ...e,
          kind: "purchase" as const,
        })),
        ...(harEvents.get(name) ?? []).map((e) => ({
          ...e,
      ...
      w(
        `**${name}** — total deficit ${remainingRows.find((r) => r.name === name)?.remaining ?? "?"}, ${flags.length} likely grant date(s):`,
      );
      if (flags.length === 0) {
        w(
          "- (deficit only from unmatched/unlogged purchases before Dec 1 — no specific over-delivery date found)",
        );
      } else {
  ...
  w("### package_orders");
  w(
    `- Real purchases missing a name (orphans, see A2): **${pkgOrphan.length}**`,
  );
  w(`- Empty template/filler rows (ignored): **${pkgFiller}**`);
  w(
    `- Named rows with Porsi = 0: **${pkgBlankPorsi.length}**${pkgBlankPorsi.length ? ` — ${pkgBlankPorsi.join("; ")}` : ""}`,
  );
  w(
    `- Suspicious Porsi > 500 (likely typo): **${pkgSuspicious.length}**${pkgSuspicious.length ? ` — ${pkgSuspicious.join("; ")}` : ""}`,
  );
  w("");
  w("### ORDER_HARIAN");
  w(`- Rows with unparseable date: **${harBlankDate.length}**`);
  w(
    `- Rows with Porsi = 0: **${harZeroPorsi.length}**${harZeroPorsi.length ? ` — ${harZeroPorsi.slice(0, 40).join("; ")}${harZeroPorsi.length > 40 ? " …" : ""}` : ""}`,
  );
  ...
  console.log(
    `  package unmatched: ${pkgUnmatched.size}, harian blank-name: ${harBlankName.length}, harian unmatched: ${harUnmatched.size}`,
  );
}
