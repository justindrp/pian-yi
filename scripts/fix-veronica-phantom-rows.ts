...
async function main() {
  ...
  for (const t of targets)
    console.log(
      `  ${t.delivery_date} ${t.meal_type} p=${t.portions} order=${t.order_id?.slice(0, 8)} id=${t.id}`,
    );
  if (targets.length !== 2)
    throw new Error(
      `expected exactly 2 rows, got ${targets.length} — aborting`,
    );
  ...
  await logEdit({
    ...
    changes: {
      reason:
        "generator wrote a full both_fixed week on 2026-06-08; the Google Sheet order_harian shows only 6/12 dinner and 6/13 lunch were delivered. DB draws 30 vs sheet 28.",
