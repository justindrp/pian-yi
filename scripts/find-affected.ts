...
async function main() {
  ...
  console.log(
    `Customers with multiple package_orders rows: ${multiPurchase.length}\n`,
  );
  ...
  const custByNameLower = new Map<
    string,
    { id: string; name: string; portions_remaining: number | null }
  >();
  ...
  for (const [sheetName, purchases] of multiPurchase) {
    ...
    if (custId) {
      const { data } = await db
        .from("orders")
        .select(
          "id, status, package_size, portions_remaining, price_per_portion, total_price, start_date, created_at",
        )
    ...
    console.log(
      `  Sheet: ${purchases.length} purchases, total ${totalPortions}p, Rp${totalPrice.toLocaleString("id-ID")}`,
    );
    for (const p of purchases) {
      console.log(
        `    - ${p.date}: ${p.porsi}p @ ${p.price}/p = Rp${p.total.toLocaleString("id-ID")}`,
      );
    }
    if (dbOrders.length === 0) {
      ...
      for (const o of dbOrders) {
        console.log(
          `    - ${o.start_date} (created ${o.created_at?.slice(0, 10)}): ${o.package_size}p @ ${o.price_per_portion}/p, remaining=${o.portions_remaining}, status=${o.status}`,
        );
      }
    }
    console.log();
