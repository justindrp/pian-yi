...
async function main() {
  ...
  const { data: hannas } = await db
    .from("customers")
    .select("id, name, phone_number, area")
    .ilike("name", "%hanna%");

  console.log("Found Hanna rows:");
  for (const h of hannas ?? [])
    console.log(`  ${h.id} | ${h.phone_number} | area=${h.area}`);
  ...
  for (const o of dupOrders ?? []) {
    console.log(
      `  ${o.id} | ${o.start_date} | ${o.package_size}p @ ${o.price_per_portion} | ${o.status}`,
    );
  }
  ...
  if (!dryRun) {
    ...
    const { count: delivCount } = await db
