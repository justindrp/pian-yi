...
async function main() {
  ...
  const byName = new Map<
    string,
    { date: string; porsi: number; price: number; total: number }[]
  >();
  ...
  for (const name of TARGETS) {
    ...
    if (!cust) {
      console.log("  DB: customer not found");
      continue;
    }
    ...
    for (const o of orders ?? []) {
      const { count } = await db
        ...
        .eq("order_id", o.id);
      console.log(
        `  ${o.start_date} (created ${o.created_at?.slice(0, 10)}): ${o.package_size}p remaining=${o.portions_remaining} status=${o.status} deliveries=${count}`,
      );
    }
  }
}
