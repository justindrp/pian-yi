...
async function main() {
  ...
  const rows = ADD.map(a => ({
    ...a, customer_id: ID, order_id: JUNE, subcontractor_id: THENIE,
    portions: 1, address_slot: 1, status: 'scheduled',
  }));
  console.log('rows to insert:'); for (const r of rows) console.log(' ', JSON.stringify(r));
  console.log(`orders.portions_remaining: ${OLD10.slice(0,8)} -2 -> 0 (phantom rows returned), ${JUNE.slice(0,8)} 4 -> 2 (two days booked)`);

  if (!APPLY) { console.log('\nDRY RUN — rerun with --apply'); return; }

  const { error: insErr } = await db.from('daily_deliveries')
    .upsert(rows, { onConflict: 'delivery_date,customer_id,meal_type' });
  if (insErr) throw insErr;

  const a = await db.from('orders').update({ portions_remaining: 0 }).eq('id', OLD10);
  if (a.error) throw a.error;
  const b = await db.from('orders').update({ portions_remaining: 2 }).eq('id', JUNE);
  if (b.error) throw b.error;

  await logEdit({
    db, actor: 'justindrp2@gmail.com', entityType: 'daily_deliveries', entityId: ID,
    action: 'restore_refused_days',
    changes: {
      reason: 'bot refused Senin 24 (cutoff passed by 2 min) and skipped Selasa 25 (global holiday table; Thenie is open). Customer asked for Senin-Sabtu.',
      added: rows,
