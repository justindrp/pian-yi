...
async function main() {
  ...
  const custMap = new Map(
    allCustomers?.map((c) => [c.name?.toLowerCase().trim(), c.id]) ?? [],
  );

  const { data: allOrders } = await db
    .from("orders")
    .select(
      "id, customer_id, package_size, portions_remaining, price_per_portion, start_date, created_at, meal_time_preference, portions_per_delivery, subcontractor_id",
    )
    .order("created_at");
  ...
  for (const fix of FIXES) {
    ...
    if (!custId) {
      console.log(`${fix.name}: customer not found — skip`);
      continue;
    }
    fix.custId = custId;
    ...
    if (fix.name === "Nicholas Satria") {
      ...
      if (importOrders.length !== 2) {
        console.log(
          `${fix.name}: expected 2 import orders, found ${importOrders.length} — skip`,
        );
        continue;
      }
      ...
      if (importOrders.length !== 1) {
        console.log(
          `${fix.name}: expected 1 import order, found ${importOrders.length} — skip`,
        );
        continue;
      }
      ...
      for (const junk of junkOrders) {
        ...
        if ((count ?? 0) === 0) fix.deleteOrderIds.push(junk.id);
          ...
          console.log(
            `  ⚠ ${fix.name}: junk order ${junk.id} has ${count} deliveries — not deleting`,
          );
      }
    }
    ...
    if (!baseOrder) {
      console.log(`${fix.name}: could not resolve import order — skip`);
      continue;
    }
    ...
    for (const p of olderPurchases) {
      console.log(
        `  + Create completed: ${p.date}, ${p.porsi}p @ ${p.price}/p = Rp${p.total.toLocaleString("id-ID")}`,
      );
      if (!dryRun) {
        const { error } = await db.from("orders").insert({
          ...
          meal_time_preference:
            baseOrder.meal_time_preference || "per_day_decision",
    ...
    console.log(
      `  ✎ Update import order ${fix.importOrderId} → ${lastPurchase.date}, ${lastPurchase.porsi}p @ ${lastPurchase.price}/p`,
    );
    if (!dryRun) {
