...
const FIXES: {
  name: string;
  purchases: { date: string; porsi: number; price: number; total: number }[];
}[] = [
  {
    name: "Vanessa",
    purchases: [
...
async function main() {
  ...
  const { data: allOrders } = await db
    .from("orders")
    .select(
      "id, customer_id, subcontractor_id, meal_time_preference, portions_per_delivery",
    )
    .order("created_at");
      ...
      if (!dryRun) {
        const { error } = await db.from("orders").insert({
          ...
          meal_time_preference:
            anyOrder?.meal_time_preference ?? "per_day_decision",
