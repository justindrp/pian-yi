...
2. **`GET /api/cron/auto-resume-bot`** — the backstop for threads whose customer never writes again. Runs every 15 min from the `cron-auto-resume-bot` Railway service. This route existed for months with no service behind it, so it had never run once; path 1 is deliberately independent of it, and covers every thread that is actually alive even if the cron breaks again.

Railway crons are plain services, not a separate resource type: image `curlimages/curl`, a `cronSchedule`, and a `startCommand` that curls the route with the `x-cron-secret` header. `railway add -i curlimages/curl -s <name>` creates one, but the CLI has no flag for the schedule or start command — set those with a `serviceInstanceUpdate` GraphQL mutation against `backboard.railway.com/graphql/v2`, then `railway redeploy`. Verify a new cron with a GraphQL read of `cronSchedule` / `startCommand`; `railway service list --json` does not return either field, so it cannot tell you whether a cron is actually scheduled.
